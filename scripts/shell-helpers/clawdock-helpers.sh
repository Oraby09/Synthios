#!/usr/bin/env bash
# ClawDock - Docker helpers for Synthios
# Inspired by Simon Willison's "Running Synthios in Docker"
# https://til.simonwillison.net/llms/synthios-docker
#
# Installation:
...
# Common paths to check for Synthios
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/synthios"
  "${HOME}/workspace/synthios"
  "${HOME}/projects/synthios"
  "${HOME}/dev/synthios"
  "${HOME}/code/synthios"
  "${HOME}/src/synthios"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Synthios at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/synthios"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Synthios not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/synthios/synthios.git ~/synthios"
    echo "  cd ~/synthios && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SYNTHIOS_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d synthios-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart synthios-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f synthios-gateway
}
...
clawdock-workspace() {
  cd ~/.synthios/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec synthios-gateway \
    bash -c 'echo "alias synthios=\"./synthios.mjs\"" > /tmp/.bashrc_synthios && bash --rcfile /tmp/.bashrc_synthios'
}
...
clawdock-cli() {
  _clawdock_compose run --rm synthios-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build synthios-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SYNTHIOS_GATEWAY_TOKEN=$token" synthios-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" synthios-gateway \
    bash -c './synthios.mjs config set gateway.remote.token "$TOKEN" && ./synthios.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec synthios-gateway \
    bash -c "./synthios.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart synthios-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm synthios-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec synthios-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'synthios config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Synthios${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (synthios alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to synthios project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.synthios)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'synthios channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'synthios status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.synthios.ai${_CLR_RESET}"
  echo ""
}
