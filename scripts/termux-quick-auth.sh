...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/synthios/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Synthios Re-Auth" -c "After getting key, run: ssh $SERVER '~/synthios/scripts/mobile-reauth.sh'" --id synthios-auth
        ;;
    *)
