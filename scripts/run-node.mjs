...
const extensionRestartMetadataFiles = new Set(["synthios.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.SYNTHIOS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.SYNTHIOS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[synthios] ${message}\n`);
};

const runSynthios = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["synthios.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
