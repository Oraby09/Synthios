...
IMAGE_NAME="synthios-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  SYNTHIOS_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  SYNTHIOS_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export SYNTHIOS_ENTRY

home_dir=$(mktemp -d "/tmp/synthios-plugins-e2e.XXXXXX")
...
JS
  cat > "$dir/synthios.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/synthios.plugin.json" "$id"
...
mkdir -p "$HOME/.synthios/extensions/demo-plugin"

cat > "$HOME/.synthios/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.synthios/extensions/demo-plugin/synthios.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$SYNTHIOS_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/synthios-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@synthios/demo-plugin-tgz",
  "version": "0.0.1",
  "synthios": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/synthios.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$SYNTHIOS_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$SYNTHIOS_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/synthios-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@synthios/demo-plugin-dir",
  "version": "0.0.1",
  "synthios": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/synthios.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$SYNTHIOS_ENTRY" plugins install "$dir_plugin"
node "$SYNTHIOS_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/synthios-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@synthios/demo-plugin-file",
  "version": "0.0.1",
  "synthios": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/synthios.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$SYNTHIOS_ENTRY" plugins install "file:$file_pack_dir/package"
node "$SYNTHIOS_ENTRY" plugins list --json > /tmp/plugins4.json
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "synthios/fixture-marketplace"
    }
  }
}
JSON

node "$SYNTHIOS_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$SYNTHIOS_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$SYNTHIOS_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$SYNTHIOS_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
const configPath = path.join(process.env.HOME, ".synthios", "synthios.json");
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$SYNTHIOS_ENTRY" plugins update marketplace-shortcut --dry-run
node "$SYNTHIOS_ENTRY" plugins update marketplace-shortcut
node "$SYNTHIOS_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
