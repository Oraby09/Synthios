...
IMAGE_NAME="synthios-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="synthios-net-e2e-$$"
GW_NAME="synthios-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "SYNTHIOS_GATEWAY_TOKEN=$TOKEN" \
  -e "SYNTHIOS_SKIP_CHANNELS=1" \
  -e "SYNTHIOS_SKIP_GMAIL_WATCHER=1" \
  -e "SYNTHIOS_SKIP_CRON=1" \
  -e "SYNTHIOS_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
