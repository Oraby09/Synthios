#!/bin/bash
# Setup Synthios Auth Management System
...
echo "=== Synthios Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'synthios-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Synthios message: Send warning via Synthios itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/synthios-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/synthios-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/synthios-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now synthios-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status synthios-auth-monitor.timer"
echo ""
echo "Quick commands:"
