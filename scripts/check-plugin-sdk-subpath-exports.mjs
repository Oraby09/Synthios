...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("synthios/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("synthios/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced synthios/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced synthios/plugin-sdk/<subpath> must exist in the public package exports.",
  );
