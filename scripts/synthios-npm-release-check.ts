...
const EXPECTED_REPOSITORY_URL = "https://github.com/synthios/synthios";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "synthios") {
    errors.push(`package.json name must be "synthios"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.synthios !== "synthios.mjs") {
    errors.push(
      `package.json bin.synthios must be "synthios.mjs"; found "${pkg.bin?.synthios ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`synthios-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `synthios-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
