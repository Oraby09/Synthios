...
# Build and bundle Synthios into a minimal .app we can open.
# Outputs to dist/Synthios.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Synthios.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Synthios"
BUNDLE_ID="${BUNDLE_ID:-ai.synthios.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/synthios/synthios/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Synthios/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SynthiosBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SynthiosGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Synthios"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Synthios"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Synthios"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Synthios" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Synthios/Resources/Synthios.icns" "$APP_ROOT/Contents/Resources/Synthios.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Synthios/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SynthiosKit resources"
SYNTHIOSKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SynthiosKit_SynthiosKit.bundle"
if [ -d "$SYNTHIOSKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SynthiosKit_SynthiosKit.bundle"
  cp -R "$SYNTHIOSKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SynthiosKit_SynthiosKit.bundle"
else
  echo "WARN: SynthiosKit resource bundle not found at $SYNTHIOSKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Synthios"
killall -q Synthios 2>/dev/null || true
