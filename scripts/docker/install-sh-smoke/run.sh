...
INSTALL_URL="${SYNTHIOS_INSTALL_URL:-https://synthios.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SYNTHIOS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SYNTHIOS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="synthios"
PACKAGE_NAME="${SYNTHIOS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SYNTHIOS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SYNTHIOS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SYNTHIOS_INSTALL_LATEST_OUT:-}"
fi
