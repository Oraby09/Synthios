...
# Synthios Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Synthios."
...
GUM_VERSION="${SYNTHIOS_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Synthios Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://synthios.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$SYNTHIOS_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.synthios.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_synthios_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.synthios-* "$npm_root"/synthios 2>/dev/null || true
}

extract_synthios_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_synthios_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/synthios" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/synthios"|"/usr/local/bin/synthios")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/synthios/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale synthios symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing synthios binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Synthios package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_synthios_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Synthios npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*synthios" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_synthios_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_synthios_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Synthios npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an synthios binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "SYNTHIOS_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "SYNTHIOS_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "SYNTHIOS_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "SYNTHIOS_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "SYNTHIOS_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "SYNTHIOS_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "SYNTHIOS_BETA" "CLAWDBOT_BETA"
map_legacy_env "SYNTHIOS_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "SYNTHIOS_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "SYNTHIOS_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "SYNTHIOS_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "SYNTHIOS_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "SYNTHIOS_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${SYNTHIOS_TAGLINE_INDEX:-}" ]]; then
        if [[ "${SYNTHIOS_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((SYNTHIOS_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${SYNTHIOS_NO_ONBOARD:-0}
NO_PROMPT=${SYNTHIOS_NO_PROMPT:-0}
DRY_RUN=${SYNTHIOS_DRY_RUN:-0}
INSTALL_METHOD=${SYNTHIOS_INSTALL_METHOD:-}
SYNTHIOS_VERSION=${SYNTHIOS_VERSION:-latest}
USE_BETA=${SYNTHIOS_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/synthios"
GIT_DIR=${SYNTHIOS_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${SYNTHIOS_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${SYNTHIOS_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${SYNTHIOS_VERBOSE:-0}"
VERIFY_INSTALL="${SYNTHIOS_VERIFY_INSTALL:-0}"
SYNTHIOS_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Synthios installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/synthios)
...
Environment variables:
  SYNTHIOS_INSTALL_METHOD=git|npm
  SYNTHIOS_VERSION=latest|next|main|<semver>|<spec>
  SYNTHIOS_BETA=0|1
  SYNTHIOS_GIT_DIR=...
  SYNTHIOS_GIT_UPDATE=0|1
  SYNTHIOS_NO_PROMPT=1
  SYNTHIOS_VERIFY_INSTALL=1
  SYNTHIOS_DRY_RUN=1
  SYNTHIOS_NO_ONBOARD=1
  SYNTHIOS_VERBOSE=1
  SYNTHIOS_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Synthios checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Synthios source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_synthios_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"synthios"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://synthios.ai/install.sh | bash"
}
...
ensure_synthios_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/synthios" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/synthios" ]]; then
        ln -sf "$npm_root/synthios/dist/entry.js" "${npm_bin}/synthios"
        ui_info "Created synthios bin link at ${npm_bin}/synthios"
    fi
    return 0
}

# Check for existing Synthios installation
check_existing_synthios() {
    if [[ -n "$(type -P synthios 2>/dev/null || true)" ]]; then
        ui_info "Existing Synthios installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make synthios show as \"command not found\" in new terminals."
...
warn_synthios_not_found() {
    ui_warn "Installed, but synthios is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t synthios 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named synthios; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_synthios_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P synthios 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/synthios" ]]; then
        echo "${npm_bin}/synthios"
        return 0
    fi
...
install_synthios_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/synthios/synthios.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Synthios from git checkout: ${repo_dir}"
    else
        ui_info "Installing Synthios from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Synthios" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Synthios" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/synthios" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/synthios"
    ui_success "Synthios wrapper installed to \$HOME/.local/bin/synthios"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view synthios dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:synthios/synthios#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_synthios() {
    local package_name="synthios"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            SYNTHIOS_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="synthios"
        else
            SYNTHIOS_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${SYNTHIOS_VERSION}" ]]; then
        SYNTHIOS_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${SYNTHIOS_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${SYNTHIOS_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Synthios v${resolved_version}"
    else
        ui_info "Installing Synthios (${SYNTHIOS_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${SYNTHIOS_VERSION}")"

    if ! install_synthios_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_synthios_paths
        install_synthios_npm "${install_spec}"
    fi

    if [[ "${SYNTHIOS_VERSION}" == "latest" && "${package_name}" == "synthios" ]]; then
        if ! resolve_synthios_bin &> /dev/null; then
            ui_warn "npm install synthios@latest failed; retrying synthios@next"
            cleanup_npm_synthios_paths
            install_synthios_npm "synthios@next"
        fi
    fi

    ensure_synthios_bin_link || true

    ui_success "Synthios installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${SYNTHIOS_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_synthios_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (synthios not on PATH yet)"
        warn_synthios_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${SYNTHIOS_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.synthios/workspace-${profile}"
    else
        echo "${HOME}/.synthios/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${SYNTHIOS_CONFIG_PATH:-$HOME/.synthios/synthios.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run synthios onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but synthios not on PATH; skipping onboarding"
        warn_synthios_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run synthios onboard to retry"
        return
    }
}
...
resolve_synthios_version() {
    local version=""
    local raw_version_output=""
    local claw="${SYNTHIOS_BIN:-}"
    if [[ -z "$claw" ]] && command -v synthios &> /dev/null; then
        claw="$(command -v synthios)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_synthios_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/synthios/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/synthios/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: synthios not on PATH yet"
        warn_synthios_not_found
        return 1
    fi

    run_quiet_step "Checking Synthios version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: synthios gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_synthios_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Synthios checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set SYNTHIOS_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_synthios; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Synthios"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g synthios &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g synthios 2>/dev/null || true
        ...
        install_synthios_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/synthios" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/synthios"
    ...
    SYNTHIOS_BIN="$(resolve_synthios_bin || true)"
    ...
    installed_version=$(resolve_synthios_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Synthios installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Synthios installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/synthios"
        ui_kv "Update command" "synthios update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running synthios doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                SYNTHIOS_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                SYNTHIOS_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                SYNTHIOS_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run synthios doctor and synthios plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run synthios onboard later"
        else
            local config_path="${SYNTHIOS_CONFIG_PATH:-$HOME/.synthios/synthios.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (synthios not on PATH yet)"
                    warn_synthios_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run synthios onboard to finish setup"
            return 0
        fi
    fi

    if command -v synthios &> /dev/null; then
        local claw="${SYNTHIOS_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_synthios_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (synthios daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if SYNTHIOS_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: synthios daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${SYNTHIOS_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
