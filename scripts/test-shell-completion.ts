 ...
 * `synthios update`. Use it to verify the completion installation flow
...
\`synthios update\`, \`synthios doctor\`, and \`synthios onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Synthios Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`synthios completion --install\` later to enable.`));
    return;
  }
