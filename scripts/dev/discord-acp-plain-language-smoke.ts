...
type DriverMode = "token" | "webhook" | "synthios";
...
function resolveStateDir(): string {
  const override = process.env.SYNTHIOS_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.SYNTHIOS_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".synthios");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver synthios] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Synthios spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|synthios> Driver transport mode (default: token)\n" +
    ...
    "  --synthios-bin <path>        Synthios CLI binary for driver=synthios (default: synthios)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  SYNTHIOS_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  SYNTHIOS_DISCORD_SMOKE_DRIVER\n" +
    "  SYNTHIOS_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  SYNTHIOS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  SYNTHIOS_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  SYNTHIOS_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  SYNTHIOS_DISCORD_SMOKE_AGENT\n" +
    "  SYNTHIOS_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  SYNTHIOS_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  SYNTHIOS_DISCORD_SMOKE_POLL_MS\n" +
    "  SYNTHIOS_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  SYNTHIOS_DISCORD_SMOKE_SYNTHIOS_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.SYNTHIOS_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.SYNTHIOS_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.SYNTHIOS_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.SYNTHIOS_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.SYNTHIOS_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.SYNTHIOS_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.SYNTHIOS_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.SYNTHIOS_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.SYNTHIOS_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.SYNTHIOS_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.SYNTHIOS_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.SYNTHIOS_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const synthiosBin =
    resolveArg("--synthios-bin") || process.env.SYNTHIOS_DISCORD_SMOKE_SYNTHIOS_BIN || "synthios";
...
async function synthiosCliJson<T>(params: { synthiosBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.synthiosBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`synthios ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await synthiosCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    synthiosBin: params.synthiosBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "synthios") {
    return await readMessagesWithOpenclaw({
      synthiosBin: params.args.synthiosBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `synthios-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await synthiosCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("synthios message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Synthios.`,
        diagnostics: {
          bindingCandidates: [
            {
