...
BASE_IMAGE="${BASE_IMAGE:-synthios-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-synthios-sandbox-common:bookworm-slim}"
...
SYNTHIOS_DOCKER_BUILD_USE_BUILDX="${SYNTHIOS_DOCKER_BUILD_USE_BUILDX:-0}"
SYNTHIOS_DOCKER_BUILD_CACHE_FROM="${SYNTHIOS_DOCKER_BUILD_CACHE_FROM:-}"
SYNTHIOS_DOCKER_BUILD_CACHE_TO="${SYNTHIOS_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${SYNTHIOS_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${SYNTHIOS_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${SYNTHIOS_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${SYNTHIOS_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${SYNTHIOS_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=synthios.sandbox=1)
NOTE
