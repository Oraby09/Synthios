...
- regenerates apps/ios/Synthios.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${SYNTHIOS_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid SYNTHIOS_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid SYNTHIOS_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid SYNTHIOS_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid SYNTHIOS_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing SYNTHIOS_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(SYNTHIOS_URL_SLASH)$(SYNTHIOS_URL_SLASH)#g'
)"
...
SYNTHIOS_CODE_SIGN_STYLE = Automatic
SYNTHIOS_DEVELOPMENT_TEAM = ${TEAM_ID}
SYNTHIOS_IOS_SELECTED_TEAM = ${TEAM_ID}
SYNTHIOS_APP_BUNDLE_ID = ai.synthios.client
SYNTHIOS_SHARE_BUNDLE_ID = ai.synthios.client.share
SYNTHIOS_ACTIVITY_WIDGET_BUNDLE_ID = ai.synthios.client.activitywidget
SYNTHIOS_WATCH_APP_BUNDLE_ID = ai.synthios.client.watchkitapp
SYNTHIOS_WATCH_EXTENSION_BUNDLE_ID = ai.synthios.client.watchkitapp.extension
SYNTHIOS_APP_PROFILE =
SYNTHIOS_SHARE_PROFILE =
SYNTHIOS_PUSH_TRANSPORT = relay
SYNTHIOS_PUSH_DISTRIBUTION = official
SYNTHIOS_URL_SLASH = /
SYNTHIOS_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
SYNTHIOS_PUSH_APNS_ENVIRONMENT = production
EOF
