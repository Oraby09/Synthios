...
#
# Suggested cron: */30 * * * * /home/admin/synthios/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Synthios notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., synthios-alerts)
...
STATE_FILE="$HOME/.synthios/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Synthios if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use synthios
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Synthios to $NOTIFY_PHONE..."
            synthios send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Synthios is down. Run: ssh l36 '~/synthios/scripts/mobile-reauth.sh'" "urgent"
    exit 1
