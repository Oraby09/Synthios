---
name: node-connect
description: Diagnose Synthios node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Synthios is advertising that route, then fix pairing/auth.
...
- whether `synthios devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `synthios qr --json`. It uses the same setup-code payload Android scans.

```bash
synthios config get gateway.mode
synthios config get gateway.bind
synthios config get gateway.tailscale.mode
synthios config get gateway.remote.url
synthios config get gateway.auth.mode
synthios config get gateway.auth.allowTailscale
synthios config get plugins.entries.device-pair.config.publicUrl
synthios qr --json
synthios devices list
synthios nodes status
```

If this Synthios instance is pointed at a remote gateway, also run:

```bash
synthios qr --remote --json
```
...
`synthios qr --json` success means:
...
If `synthios qr --json` says `Gateway is only bound to loopback`:
...
- `synthios devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run synthios qr again, rescan, then approve the pending device pairing.`
