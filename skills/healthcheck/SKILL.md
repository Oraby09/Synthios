---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Synthios deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Synthios cron scheduling for periodic checks, or version status checks on a machine running Synthios (laptop, workstation, Pi, VPS).
---

# Synthios Host Hardening

## Overview

Assess and harden the host running Synthios, then align it to a user-defined risk tolerance without breaking access. Use Synthios security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Synthios changes the host firewall, SSH, or OS updates; it does not.
...
5. Synthios gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Synthios can access sensitive data.
...
### 2) Run Synthios security audits (read-only)

As part of the default read-only checks, run `synthios security audit --deep`. Only offer alternatives if the user requests them:

1. `synthios security audit` (faster, non-probing)
2. `synthios security audit --json` (structured output)

Offer to apply Synthios safe defaults (numbered):

1. `synthios security audit --fix`

Be explicit that `--fix` only tightens Synthios defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Synthios version/update status (read-only)

As part of the default read-only checks, run `synthios update status`.
...
- Credential hygiene notes (location of Synthios creds, prefer disk encryption)
...
- Synthios security audit (re-run)
...
After Synthios install or first hardening pass, run at least one baseline audit and version check:

- `synthios security audit`
- `synthios security audit --deep`
- `synthios update status`

Ongoing monitoring is recommended. Use the Synthios cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `synthios cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `synthios update status`
...
Before creating, `synthios cron list` and match on exact `name`. If found, `synthios cron edit <id> ...`.
If not found, `synthios cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `synthios update status` (preferred for source checkouts and channels)
2. `npm view synthios version` (published npm version)

## Synthios command accuracy

Use only supported commands and flags:

- `synthios security audit [--deep] [--fix] [--json]`
- `synthios status` / `synthios status --deep`
- `synthios health --json`
- `synthios update status`
- `synthios cron add|list|runs|run`

Do not invent CLI flags or imply Synthios enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Synthios compaction:
