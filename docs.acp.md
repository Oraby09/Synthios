...
This document describes how the Synthios ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`synthios acp` exposes an ACP agent over stdio and forwards prompts to a running
Synthios Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Synthios Gateway
...
- ACP client spawns `synthios acp` and speaks ACP messages over stdio.
...
`synthios acp` resolves the Gateway URL and auth from CLI flags or config:
