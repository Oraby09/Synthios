...
enum CommandResolver {
    private static let projectRootDefaultsKey = "synthios.gatewayProjectRootPath"
    private static let helperName = "synthios"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let synthiosEntry = root.appendingPathComponent("synthios.mjs").path
        if FileManager().isReadableFile(atPath: synthiosEntry) { return synthiosEntry }
        let binEntry = root.appendingPathComponent("bin/synthios.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/synthios")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let synthiosPaths = self.synthiosManagedPaths(home: home)
        if !synthiosPaths.isEmpty {
            extras.insert(contentsOf: synthiosPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + synthiosPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func synthiosManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".synthios"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func synthiosExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSynthiosExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("synthios.mjs").path,
            root.appendingPathComponent("bin/synthios.js").path,
        ]
    ...
    static func hasAnySynthiosInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.synthiosExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func synthiosNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let synthiosPath = self.projectSynthiosExecutable(projectRoot: root) {
            return [synthiosPath, subcommand] + extraArgs
        }
        if let synthiosPath = self.synthiosExecutable(searchPaths: searchPaths) {
            return [synthiosPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "synthios", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            synthios entrypoint missing (looked for dist/index.js or synthios.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real synthios CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/synthios"
        ...
        \(cliSection)
        \(projectSection)
        if command -v synthios >/dev/null 2>&1; then
          CLI="$(command -v synthios)"
          synthios \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/synthios.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/synthios.mjs"
            node "$PRJ/synthios.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/synthios.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/synthios.js"
            node "$PRJ/bin/synthios.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent synthios"
          pnpm --silent synthios \(quotedArgs);
        else
          echo "synthios CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SynthiosConfigFile.loadDict()
