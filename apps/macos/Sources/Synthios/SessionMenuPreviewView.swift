...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.synthios", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SynthiosSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SynthiosSessionPreviewEntry,
    ...
    private static func cache(payload: SynthiosSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SynthiosChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SynthiosKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SynthiosKit.AnyCodable]) -> [SynthiosChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SynthiosChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SynthiosChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SynthiosChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SynthiosChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SynthiosChatMessage) -> String? {
