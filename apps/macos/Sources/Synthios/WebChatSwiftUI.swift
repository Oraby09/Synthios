...
private let webChatSwiftLogger = Logger(subsystem: "ai.synthios", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "synthios.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: SynthiosChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> SynthiosChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(SynthiosChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            SynthiosChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return SynthiosChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SynthiosChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SynthiosGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SynthiosAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: SynthiosProtocol.ModelChoice) -> SynthiosChatModelChoice {
        SynthiosChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SynthiosChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SynthiosChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SynthiosChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: SynthiosChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SynthiosChatView>) -> NSViewController
    {
