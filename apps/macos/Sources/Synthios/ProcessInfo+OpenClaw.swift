...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.synthios.mac`) even if the shipped
    ...
    {
        if environment["SYNTHIOS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "synthios.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "synthios.nixMode") { return true }
