...
enum DebugActions {
    private static let verboseDefaultsKey = "synthios.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "synthios.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SynthiosPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Synthios", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SynthiosPaths.configURL
        guard
