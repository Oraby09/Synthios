...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.synthios/synthios.json. Prefer SYNTHIOS_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.synthios/synthios.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.synthios/synthios.json. Restart the gateway to apply."
        }
