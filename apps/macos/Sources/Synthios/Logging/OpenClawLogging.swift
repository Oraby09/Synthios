...
enum SynthiosLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SynthiosOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SynthiosFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SynthiosLogging.bootstrapIfNeeded()
        let label = SynthiosLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SynthiosOSLogHandler: AppLogLevelBackedHandler {
...
struct SynthiosFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SynthiosLogging.parseLabel(self.label)
