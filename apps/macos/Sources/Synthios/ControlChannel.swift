...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SynthiosProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.synthios", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SynthiosKit.AnyCodable]()) {
                $0[$1.key] = SynthiosKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SynthiosProtocol.AnyCodable?) -> [String: SynthiosProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SynthiosProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SynthiosKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SynthiosProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("synthios.control.heartbeat")
    static let controlAgentEvent = Notification.Name("synthios.control.agent")
}
