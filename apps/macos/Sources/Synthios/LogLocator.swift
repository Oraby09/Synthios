...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SYNTHIOS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/synthios")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("synthios-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("synthios-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/synthios/ (rolling or stdout), or nil if none exist.
