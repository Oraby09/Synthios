...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["synthios"]
        let text = "hey synthios"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey synthios do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["synthios"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "synthios", start: 0.2, duration: 0.1, range: transcript.range(of: "synthios")),
