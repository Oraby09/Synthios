...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: SynthiosProtocol.AnyCodable(["ok": SynthiosProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: SynthiosProtocol.AnyCodable(["ok": SynthiosProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = SynthiosProtocol.AnyCodable([
            "runId": SynthiosProtocol.AnyCodable("run-1"),
            "sessionKey": SynthiosProtocol.AnyCodable("main"),
            "state": SynthiosProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: SynthiosProtocol.AnyCodable(["a": SynthiosProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
