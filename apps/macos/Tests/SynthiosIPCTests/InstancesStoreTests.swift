...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: SynthiosProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SynthiosProtocol.AnyCodable] = [
            "presence": .init([SynthiosProtocol.AnyCodable(entry)]),
        ]
        let payload = SynthiosProtocol.AnyCodable(payloadMap)
