...
struct ChatMessageBubble: View {
    let message: SynthiosChatMessage
    let style: SynthiosChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SynthiosChatTheme.userText : SynthiosChatTheme.assistantText
    ...
    private var inlineAttachments: [SynthiosChatMessageContent] {
    ...
    private var toolCalls: [SynthiosChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SynthiosChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SynthiosChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SynthiosChatTheme.onboardingAssistantBubble
        }
        return SynthiosChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SynthiosChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SynthiosChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SynthiosChatTheme.userText : SynthiosChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SynthiosChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SynthiosChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SynthiosChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SynthiosChatTheme.assistantText)
            }
        }
    }
}
