...
private let chatUILogger = Logger(subsystem: "ai.synthios", category: "SynthiosChatUI")

@MainActor
@Observable
public final class SynthiosChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [SynthiosChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [SynthiosChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [SynthiosPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SynthiosChatPendingToolCall] = []
    public private(set) var sessions: [SynthiosChatSessionEntry] = []
    private let transport: any SynthiosChatTransport
    private var sessionDefaults: SynthiosChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: SynthiosChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any SynthiosChatTransport,
    ...
    public var sessionChoices: [SynthiosChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [SynthiosChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SynthiosPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SynthiosChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SynthiosChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: SynthiosChatMessage) -> SynthiosChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SynthiosChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return SynthiosChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: SynthiosChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [SynthiosChatMessage],
        incoming: [SynthiosChatMessage]) -> [SynthiosChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [SynthiosChatMessage]) -> [SynthiosChatMessage] {
        var result: [SynthiosChatMessage] = []
    ...
    private static func dedupeKey(for message: SynthiosChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SynthiosChatMessageContent] = [
            SynthiosChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SynthiosChatAttachmentPayload in
            SynthiosChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SynthiosChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = SynthiosChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: SynthiosChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SynthiosChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SynthiosAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SynthiosChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SynthiosPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
