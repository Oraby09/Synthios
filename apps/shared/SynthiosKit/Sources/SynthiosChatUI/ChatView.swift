...
@MainActor
public struct SynthiosChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: SynthiosChatViewModel
    ...
    public init(
        viewModel: SynthiosChatViewModel,
    ...
    private var visibleMessages: [SynthiosChatMessage] {
        let base: [SynthiosChatMessage]
    ...
    private func mergeToolResults(in messages: [SynthiosChatMessage]) -> [SynthiosChatMessage] {
        ...
        for message in messages {
            ...
            let merged = SynthiosChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: SynthiosChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: SynthiosChatMessage) -> Bool {
    ...
    private func primaryText(in message: SynthiosChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: SynthiosChatMessage) -> Bool {
    ...
    private func toolCalls(in message: SynthiosChatMessage) -> [SynthiosChatMessageContent] {
    ...
    private func inlineToolResults(in message: SynthiosChatMessage) -> [SynthiosChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: SynthiosChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: SynthiosChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(SynthiosChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
