...
import Foundation
import Testing
@testable import SynthiosChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> SynthiosChatHistoryPayload
{
    SynthiosChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> SynthiosChatSessionEntry {
    SynthiosChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> SynthiosChatSessionEntry
{
    SynthiosChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> SynthiosChatModelChoice {
    SynthiosChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [SynthiosChatHistoryPayload],
    sessionsResponses: [SynthiosChatSessionsListResponse] = [],
    modelResponses: [[SynthiosChatModelChoice]] = [],
    ...
    -> (TestChatTransport, SynthiosChatViewModel)
{
...
private func sendUserMessage(_ vm: SynthiosChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, SynthiosChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SynthiosChatHistoryPayload]
    private let sessionsResponses: [SynthiosChatSessionsListResponse]
    private let modelResponses: [[SynthiosChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<SynthiosChatTransportEvent>
    private let continuation: AsyncStream<SynthiosChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<SynthiosChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SynthiosChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SynthiosChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [SynthiosChatAttachmentPayload]) async throws -> SynthiosChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return SynthiosChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SynthiosChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SynthiosChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SynthiosChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = SynthiosChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = SynthiosChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: SynthiosChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = SynthiosChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: SynthiosChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(SynthiosChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == SynthiosChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = SynthiosChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: SynthiosChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = SynthiosChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = SynthiosChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = SynthiosChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { SynthiosChatViewModel(sessionKey: "main", transport: transport) }
