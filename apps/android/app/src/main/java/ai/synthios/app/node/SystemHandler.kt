...
private const val NOTIFICATION_CHANNEL_BASE_ID = "synthios.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Synthios Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Synthios Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Synthios Active")
      }
