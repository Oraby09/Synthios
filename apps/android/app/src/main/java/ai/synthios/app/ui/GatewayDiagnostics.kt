...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Synthios Android gateway connection failure.

    Please:
    - pick one route only: same machine, same LAN, Tailscale, or public URL
    - classify this as pairing/auth, TLS trust, wrong advertised route, wrong address/port, or gateway down
    - quote the exact app status/error below
    - tell me whether `synthios devices list` should show a pending pairing request
    - if more signal is needed, ask for `synthios qr --json`, `synthios devices list`, and `synthios nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Synthios gateway diagnostics", report))
