...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: SynthiosWatchNotifyParams) -> SynthiosWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [SynthiosWatchAction]?,
        kind: String?,
        promptId: String?) -> [SynthiosWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> SynthiosWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SynthiosWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                SynthiosWatchAction(id: "approve", label: "Approve"),
                SynthiosWatchAction(id: "decline", label: "Decline", style: "destructive"),
                SynthiosWatchAction(id: "open_phone", label: "Open iPhone"),
                SynthiosWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            SynthiosWatchAction(id: "done", label: "Done"),
            SynthiosWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            SynthiosWatchAction(id: "open_phone", label: "Open iPhone"),
            SynthiosWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: SynthiosWatchRisk?,
        priority: SynthiosNotificationPriority?) -> SynthiosWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: SynthiosNotificationPriority?,
        risk: SynthiosWatchRisk?) -> SynthiosNotificationPriority?
    {
