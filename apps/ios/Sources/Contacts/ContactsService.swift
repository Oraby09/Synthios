...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SynthiosContactsSearchParams) async throws -> SynthiosContactsSearchPayload {
        ...
        return SynthiosContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SynthiosContactsAddParams) async throws -> SynthiosContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SynthiosContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SynthiosContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SynthiosContactPayload {
        SynthiosContactPayload(
