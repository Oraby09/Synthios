...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SYNTHIOS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<SynthiosTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.synthios", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = SynthiosTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> SynthiosTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return SynthiosTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> SynthiosTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> SynthiosTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return SynthiosTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> SynthiosTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = SynthiosTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: SynthiosTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: SynthiosAgentEventPayload.self
            ) else {
                continue
            }
