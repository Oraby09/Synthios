...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> SynthiosDeviceStatusPayload {
        ...
        return SynthiosDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> SynthiosDeviceInfoPayload {
        ...
        return SynthiosDeviceInfoPayload(
    ...
    private func batteryStatus() -> SynthiosBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: SynthiosBatteryState = switch device.batteryState {
        ...
        return SynthiosBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> SynthiosThermalStatusPayload {
        let state: SynthiosThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return SynthiosThermalStatusPayload(state: state)
    }

    private func storageStatus() -> SynthiosStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return SynthiosStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
