...
final class RemindersService: RemindersServicing {
    func list(params: SynthiosRemindersListParams) async throws -> SynthiosRemindersListPayload {
        ...
        let payload: [SynthiosReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SynthiosReminderPayload(
        ...
        return SynthiosRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SynthiosRemindersAddParams) async throws -> SynthiosRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SynthiosReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SynthiosRemindersAddPayload(reminder: payload)
    }
