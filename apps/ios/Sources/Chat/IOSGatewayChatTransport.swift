...
struct IOSGatewayChatTransport: SynthiosChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.synthios", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> SynthiosChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SynthiosChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SynthiosChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SynthiosChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SynthiosChatAttachmentPayload]) async throws -> SynthiosChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SynthiosChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(SynthiosChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(SynthiosGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SynthiosGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SynthiosChatEventPayload.self)
                        {
