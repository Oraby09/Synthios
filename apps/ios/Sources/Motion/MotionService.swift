...
final class MotionService: MotionServicing {
    func activities(params: SynthiosMotionActivityParams) async throws -> SynthiosMotionActivityPayload {
        ...
        let mapped: [SynthiosMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return SynthiosMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SynthiosPedometerParams) async throws -> SynthiosPedometerPayload {
        ...
        let payload: SynthiosPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SynthiosPedometerPayload(
