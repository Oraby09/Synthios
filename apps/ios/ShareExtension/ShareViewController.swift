...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.synthios.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Synthios", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Synthios gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Synthios.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "SynthiosShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Synthios is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Synthios Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("synthios-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
