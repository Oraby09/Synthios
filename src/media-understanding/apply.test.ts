...
const TEMP_MEDIA_PREFIX = "synthios-media-";
...
function createGroqAudioConfig(): SynthiosConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SynthiosConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SynthiosConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: SynthiosConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredSynthiosTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        SYNTHIOS_AGENT_DIR: isolatedAgentDir,
