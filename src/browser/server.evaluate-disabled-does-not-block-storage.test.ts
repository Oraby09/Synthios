  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          synthios: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.SYNTHIOS_GATEWAY_PORT;
    process.env.SYNTHIOS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.SYNTHIOS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.SYNTHIOS_GATEWAY_PASSWORD;
