...
const { stopSynthiosChromeMock } = vi.hoisted(() => ({
  stopSynthiosChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopSynthiosChrome: stopSynthiosChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopSynthiosChromeMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["synthios", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      synthios: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.synthios).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopSynthiosChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("synthios browser stop failed: Error: oops");
  });
});
