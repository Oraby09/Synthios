...
vi.mock("./chrome.js", () => ({
  resolveSynthiosUserDataDir: vi.fn(() => "/tmp/synthios-test/synthios/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "synthios-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveSynthiosUserDataDir).mockReturnValue(userDataDir);
