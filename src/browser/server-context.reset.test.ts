...
function localSynthiosProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "synthios",
...
function createLocalSynthiosResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localSynthiosProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveSynthiosUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-reset-"));
    const profileDir = path.join(tempRoot, "synthios");
    ...
    const ops = createLocalSynthiosResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveSynthiosUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-reset-no-own-"));
    const profileDir = path.join(tempRoot, "synthios");
    fs.mkdirSync(profileDir, { recursive: true });
