...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".synthios-profile-decorated");
}
 ...
 */
export function decorateSynthiosProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SYNTHIOS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SYNTHIOS_BROWSER_COLOR).toUpperCase();
