...
let originalSynthiosLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.SYNTHIOS_TEST_FILE_LOG;
  originalSynthiosLogLevel = process.env.SYNTHIOS_LOG_LEVEL;
  delete process.env.SYNTHIOS_TEST_FILE_LOG;
  delete process.env.SYNTHIOS_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.SYNTHIOS_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalSynthiosLogLevel === undefined) {
    delete process.env.SYNTHIOS_LOG_LEVEL;
  } else {
    process.env.SYNTHIOS_LOG_LEVEL = originalSynthiosLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
