...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds SYNTHIOS_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `synthios gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service SYNTHIOS_GATEWAY_TOKEN does not match gateway.auth.token in synthios.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.SYNTHIOS_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.SYNTHIOS_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `synthios gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
