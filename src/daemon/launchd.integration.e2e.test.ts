...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `synthios-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      SYNTHIOS_LAUNCHD_LABEL: `ai.synthios.launchd-int-${testId}`,
      SYNTHIOS_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
