...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when SYNTHIOS_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".synthios", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when SYNTHIOS_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", SYNTHIOS_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".synthios-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers SYNTHIOS_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        SYNTHIOS_PROFILE: "rescue",
        SYNTHIOS_STATE_DIR: "C:\\State\\synthios",
      },
      expected: path.join("C:\\State\\synthios", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", SYNTHIOS_PROFILE: "default" },
      expected: path.join("/home/test", ".synthios", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Synthios Gateway",
          "cd /d C:\\Projects\\synthios",
          "set NODE_ENV=production",
          "set SYNTHIOS_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\synthios",
          environment: {
            NODE_ENV: "production",
            SYNTHIOS_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\synthios\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\synthios\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Synthios Share\\node.exe" "\\\\fileserver\\Synthios Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Synthios Share\\node.exe",
            "\\\\fileserver\\Synthios Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from SYNTHIOS_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ SYNTHIOS_STATE_DIR: path.join(tmpDir, "custom-state") }),
