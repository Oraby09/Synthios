...
const TEST_MANAGED_HOME = "/tmp/synthios-test-home";
const GATEWAY_SERVICE = "synthios-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/synthios gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/synthios", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/synthios-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "synthios-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "synthios-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/synthios-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled synthios-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled synthios-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when SYNTHIOS_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/synthios-gateway.service",
    },
    {
      name: "uses profile-specific service name when SYNTHIOS_PROFILE is set to a custom value",
      env: { HOME: "/home/test", SYNTHIOS_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/synthios-gateway-jbphoenix.service",
    },
    {
      name: "prefers SYNTHIOS_SYSTEMD_UNIT over SYNTHIOS_PROFILE",
      env: {
        HOME: "/home/test",
        SYNTHIOS_PROFILE: "jbphoenix",
        SYNTHIOS_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles SYNTHIOS_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        SYNTHIOS_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from SYNTHIOS_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        SYNTHIOS_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/synthios gateway start --name "My Bot"')).toEqual([
      "/usr/bin/synthios",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('synthios --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["synthios", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('synthios --path "C:\\\\Program Files\\\\Synthios"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["synthios", "--path", "C:\\\\Program Files\\\\Synthios"]);

    expect(
      splitArgsPreservingQuotes('synthios --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["synthios", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/synthios gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads SYNTHIOS_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/synthios gateway run", "EnvironmentFile=%h/.synthios/.env"],
      { [`${TEST_SERVICE_HOME}/.synthios/.env`]: "SYNTHIOS_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.SYNTHIOS_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/synthios gateway run",
        "EnvironmentFile=%h/.synthios/.env",
        'Environment="SYNTHIOS_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.synthios/.env`]: "SYNTHIOS_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.SYNTHIOS_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.SYNTHIOS_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.synthios/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/synthios-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/synthios gateway run",
          'EnvironmentFile=%h/.synthios/first.env "%h/.synthios/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.synthios/first.env") {
        return "SYNTHIOS_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.synthios/second env.env") {
        return 'SYNTHIOS_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      SYNTHIOS_GATEWAY_TOKEN: "first-token",
      SYNTHIOS_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "SYNTHIOS_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "SYNTHIOS_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "SYNTHIOS_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      SYNTHIOS_GATEWAY_TOKEN: "override-token",
      SYNTHIOS_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/synthios-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/synthios gateway run",
          "EnvironmentFile=%h/.synthios/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.synthios/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'SYNTHIOS_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "SYNTHIOS_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      SYNTHIOS_GATEWAY_TOKEN: "quoted token",
      SYNTHIOS_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      SYNTHIOS_GATEWAY_TOKEN: "file",
      SYNTHIOS_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "synthios-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ SYNTHIOS_PROFILE: "work" });
  });
