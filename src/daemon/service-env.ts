...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.SYNTHIOS_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    SYNTHIOS_PROFILE: profile,
    SYNTHIOS_GATEWAY_PORT: String(port),
    SYNTHIOS_LAUNCHD_LABEL: resolvedLaunchdLabel,
    SYNTHIOS_SYSTEMD_UNIT: systemdUnit,
    SYNTHIOS_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    SYNTHIOS_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    SYNTHIOS_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform, extraPathDirs);
  const gatewayToken =
    env.SYNTHIOS_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    SYNTHIOS_GATEWAY_TOKEN: gatewayToken,
    SYNTHIOS_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    SYNTHIOS_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    SYNTHIOS_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    SYNTHIOS_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    SYNTHIOS_LOG_PREFIX: "node",
    SYNTHIOS_SERVICE_MARKER: NODE_SERVICE_MARKER,
    SYNTHIOS_SERVICE_KIND: NODE_SERVICE_KIND,
    SYNTHIOS_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    SYNTHIOS_STATE_DIR: sharedEnv.stateDir,
    SYNTHIOS_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.SYNTHIOS_STATE_DIR;
  const configPath = env.SYNTHIOS_CONFIG_PATH;
