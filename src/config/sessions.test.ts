...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ SYNTHIOS_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-synthios",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-synthios#general");
  });
  ...
  it("derives session transcripts dir from SYNTHIOS_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SYNTHIOS_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when SYNTHIOS_STATE_DIR differs from stored paths", () => {
