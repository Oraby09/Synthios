...
import { buildWebSearchProviderConfig, withTempHome, writeSynthiosConfig } from "./test-helpers.js";
import { SynthiosSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = SynthiosSchema.safeParse({
      $schema: "https://synthios.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://synthios.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = SynthiosSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = SynthiosSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeSynthiosConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeSynthiosConfig(home, {
        gateway: { bind: "${SYNTHIOS_BIND}" },
      });

      const prev = process.env.SYNTHIOS_BIND;
      process.env.SYNTHIOS_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.SYNTHIOS_BIND;
        } else {
          process.env.SYNTHIOS_BIND = prev;
        }
      }
    });
  });
