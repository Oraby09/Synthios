...
import type { SynthiosConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareSynthiosVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  synthios config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  synthios config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: SynthiosConfig,
  pathSegments: string[],
): { changed: boolean; next: SynthiosConfig } {
...
function coerceConfig(value: unknown): SynthiosConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SynthiosConfig;
}
...
function stampConfigVersion(cfg: SynthiosConfig): SynthiosConfig {
...
function warnIfConfigFromFuture(cfg: SynthiosConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSynthiosVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Synthios (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as SynthiosConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SynthiosConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SynthiosConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SynthiosConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as SynthiosConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SYNTHIOS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SYNTHIOS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.SYNTHIOS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.SYNTHIOS_WATCH_SESSION === "string" &&
        deps.env.SYNTHIOS_WATCH_SESSION.trim().length > 0
          ? deps.env.SYNTHIOS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.SYNTHIOS_WATCH_COMMAND === "string" &&
        deps.env.SYNTHIOS_WATCH_COMMAND.trim().length > 0
          ? deps.env.SYNTHIOS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `SYNTHIOS_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: SynthiosConfig | null = null;
let runtimeConfigSourceSnapshot: SynthiosConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SYNTHIOS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SYNTHIOS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): SynthiosConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): SynthiosConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: SynthiosConfig;
  candidate: SynthiosConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: SynthiosConfig): SynthiosConfig {
...
export function loadConfig(): SynthiosConfig {
...
export async function readBestEffortConfig(): Promise<SynthiosConfig> {
...
export async function writeConfigFile(
  ...
  // edits to synthios.json remain visible (no stale snapshot).
}
