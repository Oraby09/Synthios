...
function collectConfigEnvVarsByTarget(cfg?: SynthiosConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: SynthiosConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: SynthiosConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: SynthiosConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.SYNTHIOS_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
