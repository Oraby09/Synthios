...
import type { SynthiosConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SynthiosConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: SynthiosConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: SynthiosConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SynthiosSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SynthiosConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SynthiosConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as SynthiosConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as SynthiosConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; Synthios mapped it automatically for compatibility.",
  }));
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Synthios config schema.
