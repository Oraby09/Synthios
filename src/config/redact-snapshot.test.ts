...
import type { ConfigFileSnapshot } from "./types.synthios.js";
import { SynthiosSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(SynthiosSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.synthios/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/synthios/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/synthios/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/synthios/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/synthios/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = SynthiosSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "SynthiosConfig";
    const hints = mainSchemaHints;
