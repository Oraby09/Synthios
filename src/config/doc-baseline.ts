...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.SYNTHIOS_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveSynthiosPackageRootSync({
...
function loadPackageChannelMetadata(rootDir: string): PackageChannelMetadata | null {
  try {
    ...
    const channel = packageJson.synthios?.channel;
    if (!channel) {
      return null;
    }
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    SYNTHIOS_STATE_DIR: path.join(os.tmpdir(), "synthios-config-doc-baseline-state"),
    SYNTHIOS_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
  ...
  const loadChannelsSequentiallyForDebug = process.env.SYNTHIOS_CONFIG_DOC_BASELINE_DEBUG === "1";
