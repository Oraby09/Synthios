...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("synthios-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "synthios-sandbox-browser-global" },
      agentBrowser: { network: "synthios-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("synthios-sandbox-browser-agent");
  });
