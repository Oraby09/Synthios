import { hasAnyWhatsAppAuth } from "synthios/plugin-sdk/whatsapp";
...
function isWhatsAppConfigured(cfg: SynthiosConfig): boolean {
...
function isGenericChannelConfigured(cfg: SynthiosConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return hasMeaningfulChannelConfig(entry);
}
...
function collectModelRefs(cfg: SynthiosConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SynthiosConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: SynthiosConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: SynthiosConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SynthiosConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: SynthiosConfig, pluginId: string): SynthiosConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
