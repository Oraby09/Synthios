...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.SYNTHIOS_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.SYNTHIOS_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set SYNTHIOS_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: synthios doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__SYNTHIOS_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
