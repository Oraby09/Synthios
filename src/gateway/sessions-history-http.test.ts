...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-session-history-"));
  ...
  test("returns session history over direct REST", async () => {
    ...
    try {
      ...
      expect(
        (
          body.messages?.[0] as {
            __synthios?: { id?: string; seq?: number };
          }
        )?.__synthios,
      ).toMatchObject({
        seq: 1,
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    try {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __synthios?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __synthios?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__synthios?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __synthios?: { seq?: number } }>;
        messages?: Array<{ __synthios?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__synthios?.seq)).toEqual([1]);
  ...
  test("streams session history updates over SSE", async () => {
    ...
    try {
      ...
      expect(
        (
          messageEvent.data as {
            message?: { __synthios?: { id?: string; seq?: number } };
          }
        ).message?.__synthios,
      ).toMatchObject({
