...
let lastCreateSynthiosToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/synthios-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateSynthiosToolsContext?.agentTo,
          agentThreadId: lastCreateSynthiosToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createSynthiosTools: (ctx: Record<string, unknown>) => {
      lastCreateSynthiosToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateSynthiosToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateSynthiosToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSynthiosToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-synthios-message-to": "channel:24514",
        "x-synthios-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
