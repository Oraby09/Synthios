...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("synthios doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with SYNTHIOS_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with SYNTHIOS_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.SYNTHIOS_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://synthios-gateway.ai:18789",
      onConnectError,
    });
