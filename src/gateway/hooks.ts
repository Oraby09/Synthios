...
export function resolveHooksConfig(cfg: SynthiosConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SynthiosConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-synthios-token"] === "string"
      ? req.headers["x-synthios-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-synthios-idempotency-key"]) ||
