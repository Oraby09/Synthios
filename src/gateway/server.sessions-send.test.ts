...
const { createSynthiosTools } = await import("../agents/synthios-tools.js");
...
type SessionSendTool = ReturnType<typeof createSynthiosTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createSynthiosTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["SYNTHIOS_GATEWAY_PORT", "SYNTHIOS_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.SYNTHIOS_GATEWAY_PORT = String(gatewayPort);
  process.env.SYNTHIOS_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("SYNTHIOS_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createSynthiosTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
