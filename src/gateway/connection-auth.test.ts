...
function cfg(input: Partial<SynthiosConfig>): SynthiosConfig {
  return input as SynthiosConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: SynthiosConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  SYNTHIOS_GATEWAY_TOKEN: "env-token",
  SYNTHIOS_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves config-first token SecretRef even when SYNTHIOS env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when SYNTHIOS env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
