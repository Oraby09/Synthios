...
import { createSynthiosTools } from "../agents/synthios-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-synthios-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-synthios-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-synthios-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-synthios-thread-id")?.trim() || undefined;
  ...
  const allTools = createSynthiosTools({
