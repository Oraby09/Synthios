...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "SYNTHIOS_SKIP_BROWSER_CONTROL_SERVER",
  "SYNTHIOS_SKIP_GMAIL_WATCHER",
  "SYNTHIOS_SKIP_CANVAS_HOST",
  "SYNTHIOS_BUNDLED_PLUGINS_DIR",
  "SYNTHIOS_SKIP_CHANNELS",
  "SYNTHIOS_SKIP_PROVIDERS",
  "SYNTHIOS_SKIP_CRON",
  "SYNTHIOS_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SYNTHIOS_STATE_DIR = path.join(tempHome, ".synthios");
  delete process.env.SYNTHIOS_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.SYNTHIOS_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.SYNTHIOS_SKIP_GMAIL_WATCHER = "1";
  process.env.SYNTHIOS_SKIP_CANVAS_HOST = "1";
  process.env.SYNTHIOS_SKIP_CHANNELS = "1";
  process.env.SYNTHIOS_SKIP_PROVIDERS = "1";
  process.env.SYNTHIOS_SKIP_CRON = "1";
  process.env.SYNTHIOS_TEST_MINIMAL_GATEWAY = "1";
  process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "synthios-test-no-bundled-extensions")
    : "synthios-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".synthios-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".synthios-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__synthiosTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__synthiosTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["SYNTHIOS_GATEWAY_TOKEN"]);
  const prev = process.env.SYNTHIOS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SYNTHIOS_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.SYNTHIOS_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
