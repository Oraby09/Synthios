...
export function attachSynthiosTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__synthios && typeof record.__synthios === "object" && !Array.isArray(record.__synthios)
      ? (record.__synthios as Record<string, unknown>)
      : {};
  return {
    ...record,
    __synthios: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachSynthiosTranscriptMeta(parsed.message, {
...
export function resolveSessionTranscriptCandidates(
  ...
  const legacyDir = path.join(home, ".synthios", "sessions");
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "synthios" && model === "delivery-mirror";
