...
import { type SynthiosConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: SynthiosConfig): string[] {
...
export function listAgentsForGateway(cfg: SynthiosConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: SynthiosConfig): string {
...
function resolveSessionStoreAgentId(cfg: SynthiosConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: SynthiosConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: SynthiosConfig): GatewaySessionsDefaults {
