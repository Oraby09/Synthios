    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "synthios node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["synthios node status", "Check node host service status."],
          ["synthios node install", "Install the node host service."],
          ["synthios node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.synthios.ai/cli/node")}\n`,
    );
