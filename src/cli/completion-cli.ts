...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Synthios Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(synthios completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Synthios Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(synthios completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.synthios.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $SYNTHIOS_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "synthios") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
