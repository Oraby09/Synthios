...
const SANDBOX_EXAMPLES = {
  main: [
    ["synthios sandbox list", "List all sandbox containers."],
    ["synthios sandbox list --browser", "List only browser containers."],
    ["synthios sandbox recreate --all", "Recreate all containers."],
    ["synthios sandbox recreate --session main", "Recreate a specific session."],
    ["synthios sandbox recreate --agent mybot", "Recreate agent containers."],
    ["synthios sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["synthios sandbox list", "List all sandbox containers."],
    ["synthios sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["synthios sandbox recreate --all", "Recreate all containers."],
    ["synthios sandbox recreate --session main", "Recreate a specific session."],
    ["synthios sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["synthios sandbox recreate --browser --all", "Recreate only browser containers."],
    ["synthios sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["synthios sandbox explain", "Show effective sandbox config."],
    ["synthios sandbox explain --session agent:main:main", "Explain a specific session."],
    ["synthios sandbox explain --agent work", "Explain an agent sandbox."],
    ["synthios sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.synthios.ai/cli/sandbox")}\n`,
    )
    .action(() => {
