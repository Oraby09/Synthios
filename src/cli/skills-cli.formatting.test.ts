...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-bundled-skills-test-"));
    process.env.SYNTHIOS_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
