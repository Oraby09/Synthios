      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["synthios gateway run", "Run the gateway in the foreground."],
            ["synthios gateway status", "Show service status and probe reachability."],
            ["synthios gateway discover", "Find local and wide-area gateway beacons."],
            ["synthios gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.synthios.ai/cli/gateway")}\n`,
      ),
  );
