...
beforeEach(() => {
  ...
  originalHideBanner = process.env.SYNTHIOS_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.SYNTHIOS_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.SYNTHIOS_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("synthios");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "synthios", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("synthios-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "synthios", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "synthios", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "synthios", "channels", "add"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "synthios", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.SYNTHIOS_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "synthios", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "synthios", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "synthios", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "synthios", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "synthios", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "synthios", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "synthios", "backup", "create", "--json"],
    });
