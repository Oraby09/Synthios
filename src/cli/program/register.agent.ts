...
${formatHelpExamples([
  ['synthios agent --to +15555550123 --message "status update"', "Start a new session."],
  ['synthios agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'synthios agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'synthios agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['synthios agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'synthios agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.synthios.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.synthios.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['synthios agents set-identity --agent main --name "Synthios" --emoji "🦞"', "Set name + emoji."],
  ["synthios agents set-identity --agent main --avatar avatars/synthios.png", "Set avatar path."],
  [
    "synthios agents set-identity --workspace ~/.synthios/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "synthios agents set-identity --identity-file ~/.synthios/workspace/IDENTITY.md --agent main",
