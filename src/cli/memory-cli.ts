    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["synthios memory status", "Show index and provider status."],
          ["synthios memory status --deep", "Probe embedding provider readiness."],
          ["synthios memory index --force", "Force a full reindex."],
          ['synthios memory search "meeting notes"', "Quick search using positional query."],
          [
            'synthios memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["synthios memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.synthios.ai/cli/memory")}\n`,
    );
