...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    SYNTHIOS_STATE_DIR: "/tmp/synthios-daemon",
    SYNTHIOS_CONFIG_PATH: "/tmp/synthios-daemon/synthios.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.SYNTHIOS_STATE_DIR ?? "/tmp/synthios-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.SYNTHIOS_CONFIG_PATH ?? `${stateDir}/synthios.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/synthios-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.SYNTHIOS_STATE_DIR = "/tmp/synthios-cli";
    process.env.SYNTHIOS_CONFIG_PATH = "/tmp/synthios-cli/synthios.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.SYNTHIOS_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.SYNTHIOS_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.SYNTHIOS_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "synthios-gateway" }],
        hints: [],
      },
      healthy: false,
