...
import type { SynthiosConfig, ConfigFileSnapshot } from "../config/types.synthios.js";
...
vi.mock("../infra/synthios-root.js", () => ({
  resolveSynthiosPackageRoot: vi.fn(),
}));
...
const { resolveSynthiosPackageRoot } = await import("../infra/synthios-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/synthios-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as SynthiosConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/synthios-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveSynthiosPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("synthios-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("synthios-updated-root");
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveSynthiosPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("synthios");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/synthios-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "synthios-gateway" }],
      hints: [],
    });
    ...
    const cases = [
      {
        ...
        assert: () => {
          ...
          expect(logs.join("\n")).toContain("Synthios update status");
        },
      },
      {
        name: "json output",
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as SynthiosConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "synthios@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("synthios-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "synthios@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:synthios/synthios#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("synthios-update"));
          await updateCommand({ yes: true, tag: "github:synthios/synthios#main" });
        },
        expectedSpec: "github:synthios/synthios#main",
      },
      {
        name: "SYNTHIOS_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("synthios-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/synthios-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("synthios");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("synthios-update");
    const localAppData = createCaseDir("synthios-localappdata");
  ...
  it.each([
    ...
    },
    {
      ...
      invoke: async () => {
        await withEnvAsync(
          {
            SYNTHIOS_STATE_DIR: "./state",
            SYNTHIOS_CONFIG_PATH: "./config/synthios.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            SYNTHIOS_STATE_DIR: path.resolve("./state"),
            SYNTHIOS_CONFIG_PATH: path.resolve("./config/synthios.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            SYNTHIOS_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ SYNTHIOS_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.SYNTHIOS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("synthios-update-wizard");
    await withEnvAsync({ SYNTHIOS_GIT_DIR: tempDir }, async () => {
      setTty(true);
