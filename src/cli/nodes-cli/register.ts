    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["synthios nodes status", "List known nodes with live status."],
          ["synthios nodes pairing pending", "Show pending node pairing requests."],
          ['synthios nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["synthios nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.synthios.ai/cli/nodes")}\n`,
    );
