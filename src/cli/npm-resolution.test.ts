...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@synthios/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@synthios/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@synthios/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@synthios/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @synthios/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@synthios/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@synthios/plugin-alpha@1.2.3",
        installPath: "/tmp/synthios/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@synthios/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @synthios/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@synthios/plugin-alpha@latest",
      true,
      "/tmp/synthios/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@synthios/plugin-alpha@latest",
      installPath: "/tmp/synthios/extensions/alpha",
      version: "1.2.3",
