...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Synthios and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["synthios update", "Update a source checkout (git)"],
        ["synthios update --channel beta", "Switch to beta channel (git + npm)"],
        ["synthios update --channel dev", "Switch to dev channel (git + npm)"],
        ["synthios update --tag beta", "One-off update to a dist-tag or version"],
        ["synthios update --tag main", "One-off package install from GitHub main"],
        ["synthios update --dry-run", "Preview actions without changing anything"],
        ["synthios update --no-restart", "Update without restarting the service"],
        ["synthios update --json", "Output result as JSON"],
        ["synthios update --yes", "Non-interactive (accept downgrade prompts)"],
        ["synthios update wizard", "Interactive update wizard"],
        ["synthios --update", "Shorthand for synthios update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run synthios update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.synthios.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["synthios update status", "Show channel + version status."],
          ["synthios update status --json", "JSON output."],
          ["synthios update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.synthios.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
