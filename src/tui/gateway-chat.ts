...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set SYNTHIOS_GATEWAY_TOKEN/SYNTHIOS_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "synthios-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.SYNTHIOS_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.SYNTHIOS_GATEWAY_PASSWORD);
