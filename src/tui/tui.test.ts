...
describe("resolveInitialTuiAgentId", () => {
  const cfg: SynthiosConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/synthios" },
        { id: "ops", workspace: "/tmp/synthios/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/synthios/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run synthios devices list");
    expect(state.pairingHint).toContain("synthios devices list");
  });
