...
function telegramCfg(): SynthiosConfig {
  return { channels: { telegram: { botToken: "tok" } } } as SynthiosConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: SynthiosConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as SynthiosConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as SynthiosConfig;
...
function createSignalAccountOverrideCfg(): SynthiosConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("derives discord action listings from channel and moderation gates", () => {
    const cases = [
      {
        name: "defaults",
        cfg: { channels: { discord: { token: "d0" } } } as SynthiosConfig,
        expectUploads: true,
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as SynthiosConfig,
        accountId: undefined,
        params: {
