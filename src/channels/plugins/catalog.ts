...
import { discoverSynthiosPlugins } from "../../plugins/discovery.js";
import { loadPluginManifest } from "../../plugins/manifest.js";
import type { SynthiosPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, SynthiosPackageManifest>>;

const ENV_CATALOG_PATHS = ["SYNTHIOS_PLUGIN_CATALOG_PATHS", "SYNTHIOS_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<SynthiosPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: SynthiosPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  origin?: PluginOrigin;
  workspaceDir?: string;
  packageManifest?: SynthiosPackageManifest;
...
function loadBundledMetadataCatalogEntries(options: CatalogOptions): ChannelPluginCatalogEntry[] {
  ...
  for (const dirent of fs.readdirSync(bundledDir, { withFileTypes: true })) {
    ...
    const entry = buildCatalogEntry({
      ...
      packageManifest: packageJson.synthios,
    });
    if (entry) {
      entries.push(entry);
    }
