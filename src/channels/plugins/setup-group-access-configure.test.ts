...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: SynthiosConfig, policy: ChannelAccessPolicy) => SynthiosConfig;
  resolveAllowlist: (params: { cfg: SynthiosConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: SynthiosConfig; resolved: TResolved }) => SynthiosConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: SynthiosConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: SynthiosConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: SynthiosConfig, policy: ChannelAccessPolicy): SynthiosConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: SynthiosConfig, policy: ChannelAccessPolicy): SynthiosConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: SynthiosConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: SynthiosConfig; resolved: string[] }) => {
      calls.push("apply");
