...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: SynthiosConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: SynthiosConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: SynthiosConfig, policy: DmPolicy, accountId?: string) => SynthiosConfig;
