...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: SynthiosConfig) => Array<string | number> | undefined;
}): SynthiosConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: SynthiosConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: SynthiosConfig) => Array<string | number> | undefined;
}): (cfg: SynthiosConfig, dmPolicy: DmPolicy) => SynthiosConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: SynthiosConfig, allowFrom: string[]) => SynthiosConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: SynthiosConfig, groupPolicy: "open" | "allowlist" | "disabled") => SynthiosConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: SynthiosConfig) => SynthiosConfig | Promise<SynthiosConfig>;
  applySet?: (
    cfg: SynthiosConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => SynthiosConfig | Promise<SynthiosConfig>;
}): Promise<{
  cfg: SynthiosConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in Synthios config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "SYNTHIOS_SECRET",
      envVarFormatError:
        'Use an env var name like "SYNTHIOS_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends SynthiosConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: SynthiosConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: SynthiosConfig) => Array<string | number>;
