...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    synthiosHome: process.env.SYNTHIOS_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("SYNTHIOS_HOME", snapshot.synthiosHome);
  restoreKey("SYNTHIOS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.SYNTHIOS_STATE_DIR = join(home, ".synthios");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "synthios-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".synthios", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): SynthiosConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "synthios"),
