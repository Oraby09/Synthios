...
const workspaceHarness = createCommandWorkspaceHarness("synthios-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".synthios", "extensions", params.pluginId);
...
function buildCfg(): SynthiosConfig {
  return {
    commands: {
      text: true,
      plugins: true,
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("synthios-command-plugins-home-", async () => {
