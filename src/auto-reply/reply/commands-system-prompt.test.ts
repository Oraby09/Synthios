...
const { createSynthiosCodingToolsMock } = vi.hoisted(() => ({
  createSynthiosCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/pi-tools.js", () => ({
  createSynthiosCodingTools: createSynthiosCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    createSynthiosCodingToolsMock.mockClear();
    createSynthiosCodingToolsMock.mockReturnValue([]);
  });

  it("opts command tool builds into gateway subagent binding", async () => {
    await resolveCommandsSystemPromptBundle(makeParams());

    expect(createSynthiosCodingToolsMock).toHaveBeenCalledWith(
