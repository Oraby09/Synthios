...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop synthios!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@synthios_bot", { botUsername: "synthios_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@synthios_bot", { botUsername: "synthios_bot" })).toBe(true);
