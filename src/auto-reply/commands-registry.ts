...
export function isCommandEnabled(cfg: SynthiosConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SynthiosConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: SynthiosConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SynthiosConfig) {
