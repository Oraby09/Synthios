...
declare const __SYNTHIOS_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "synthios";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["SYNTHIOS_VERSION"],
      runtimeVersion,
      env["SYNTHIOS_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Synthios version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __SYNTHIOS_VERSION__ === "string" ? __SYNTHIOS_VERSION__ : undefined,
  bundledVersion: process.env.SYNTHIOS_BUNDLED_VERSION,
});
