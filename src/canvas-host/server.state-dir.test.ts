...
describe("canvas host state dir defaults", () => {
  it("uses SYNTHIOS_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("synthios-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Synthios Canvas");
      } finally {
        await handler.close();
      }
    });
