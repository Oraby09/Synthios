...
type PluginLoadConfig = NonNullable<Parameters<typeof loadSynthiosPlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "synthios-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "synthios.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadSynthiosPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          synthios: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadSynthiosPlugins>) {
...
function useNoBundledPlugins() {
  process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadSynthiosPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadSynthiosPlugins>[0], "cache" | "config">,
) {
  return loadSynthiosPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "synthios.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
function createPluginSdkAliasFixture(params?: {
  ...
  const packageJson: Record<string, unknown> = {
    name: params?.packageName ?? "synthios",
    type: "module",
  };
  if (trustedRootIndicatorMode === "bin+marker") {
    packageJson.bin = {
      synthios: "synthios.mjs",
    };
  }
  ...
  if (trustedRootIndicatorMode === "bin+marker") {
    fs.writeFileSync(path.join(root, "synthios.mjs"), "export {};\n", "utf-8");
  }
...
function createPluginRuntimeAliasFixture(params?: { srcBody?: string; distBody?: string }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "synthios", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(
    srcFile,
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".synthios", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ SYNTHIOS_STATE_DIR: stateDir, ...params.env }, () =>
    loadSynthiosPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadSynthiosPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadSynthiosPlugins>;
  loadSecond: () => ReturnType<typeof loadSynthiosPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadSynthiosPlugins>;
  loadVariant: () => ReturnType<typeof loadSynthiosPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const synthiosHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(synthiosHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  ...
  const env = {
    ...process.env,
    SYNTHIOS_HOME: synthiosHome,
    HOME: ignoredHome,
    SYNTHIOS_STATE_DIR: stateDir,
    CLAWDBOT_STATE_DIR: undefined,
    SYNTHIOS_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".synthios", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ SYNTHIOS_STATE_DIR: stateDir }, () =>
      loadSynthiosPlugins({
        workspaceDir,
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadSynthiosPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadSynthiosPlugins(options);
          const scoped = loadSynthiosPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadSynthiosPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadSynthiosPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadSynthiosPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadSynthiosPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadSynthiosPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadSynthiosPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadSynthiosPlugins({
              ...
              env: {
                ...process.env,
                SYNTHIOS_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadSynthiosPlugins({
              ...
              env: {
                ...process.env,
                SYNTHIOS_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadSynthiosPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                SYNTHIOS_HOME: undefined,
                SYNTHIOS_STATE_DIR: stateDir,
                SYNTHIOS_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadSynthiosPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(synthiosHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadSynthiosPlugins(options),
          loadVariant: () =>
            loadSynthiosPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadSynthiosPlugins({
      env: {
        ...
        HOME: homeDir,
        SYNTHIOS_HOME: undefined,
        SYNTHIOS_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers SYNTHIOS_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const synthiosHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "synthios-home-demo",
      dir: path.join(synthiosHome, "plugins", "synthios-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "synthios-home-demo", register() {} };`,
    });
    ...
    const registry = loadSynthiosPlugins({
      ...
      config: {
        plugins: {
          allow: ["synthios-home-demo"],
          entries: {
            "synthios-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/synthios-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "synthios-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadSynthiosPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadSynthiosPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "synthios.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadSynthiosPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@synthios/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@synthios/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@synthios/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@synthios/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "synthios.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "synthios.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ SYNTHIOS_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".synthios", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("synthios/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadSynthiosPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadSynthiosPlugins({
        cache: false,
  ...
  it.each([
    ...
    },
    {
      ...
      buildFixture: () => createPluginSdkAliasFixture(),
      modulePath: () => "/tmp/tsx-cache/synthios-loader.js",
      argv1: (root: string) => path.join(root, "synthios.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("does not derive plugin-sdk subpaths from cwd fallback when package root is not an Synthios root", () => {
    const fixture = createPluginSdkAliasFixture({
      packageName: "moltbot",
  ...
  it("does not resolve plugin-sdk alias files from cwd fallback when package root is not an Synthios root", () => {
    const fixture = createPluginSdkAliasFixture({
      srcFile: "channel-runtime.ts",
      distFile: "channel-runtime.js",
      packageName: "moltbot",
...
`,
    ...
    const withAlias = createJiti(jitiBaseUrl, {
      ...__testing.buildPluginLoaderJitiOptions({
        "synthios/plugin-sdk/channel-runtime": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
  ...
  it("loads git-style package extension entries through the plugin loader when they import plugin-sdk channel-runtime (#49806)", () => {
    ...
    fs.writeFileSync(
      path.join(gitExtensionRoot, "synthios.plugin.json"),
      JSON.stringify(
        {
          id: pluginId,
          configSchema: EMPTY_PLUGIN_SCHEMA,
