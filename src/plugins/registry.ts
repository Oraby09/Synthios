...
import type {
  ...
  SynthiosPluginApi,
  SynthiosPluginChannelRegistration,
  SynthiosPluginCliRegistrar,
  SynthiosPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  SynthiosPluginHttpRouteAuth,
  SynthiosPluginHttpRouteMatch,
  SynthiosPluginHttpRouteHandler,
  SynthiosPluginHttpRouteParams,
  SynthiosPluginHookOptions,
  MediaUnderstandingProviderPlugin,
  ProviderPlugin,
  SynthiosPluginService,
  SynthiosPluginToolContext,
  SynthiosPluginToolFactory,
  PluginConfigUiHint,
  PluginDiagnostic,
  PluginBundleFormat,
  PluginFormat,
...
export type PluginToolRegistration = {
  pluginId: string;
  pluginName?: string;
  factory: SynthiosPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
  rootDir?: string;
...
export type PluginCliRegistration = {
  pluginId: string;
  pluginName?: string;
  register: SynthiosPluginCliRegistrar;
  commands: string[];
  source: string;
  rootDir?: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: SynthiosPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: SynthiosPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: SynthiosPluginHookOptions | undefined,
    config: SynthiosPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: SynthiosPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: SynthiosPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as SynthiosPluginChannelRegistration).plugin === "object"
        ? (registration as SynthiosPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: SynthiosPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: SynthiosPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: SynthiosPluginCommandDefinition) => {
