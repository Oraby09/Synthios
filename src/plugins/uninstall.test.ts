...
function createSinglePluginWithEmptySlotsConfig(): SynthiosConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): SynthiosConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "synthios-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-synthios-extensions/my-plugin",
      },
      extensionsDir,
    });
