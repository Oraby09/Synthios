...
const APPROVALS_PATH = "~/.synthios/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "synthios-app-server:thread:",
  "synthios-codex-app-server:thread:",
] as const;
...
const pluginBindingGlobalStateKey = Symbol.for("synthios.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Synthios instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
