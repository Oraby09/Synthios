...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".synthios", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".synthios", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    ...
    const homeDir = path.resolve(path.sep, "tmp", "synthios-home");
    const roots = withEnv(
      {
        SYNTHIOS_BUNDLED_PLUGINS_DIR: path.join(ignoredHome, "ignored-bundled"),
        SYNTHIOS_STATE_DIR: path.join(ignoredHome, "ignored-state"),
        HOME: ignoredHome,
      },
      () =>
        resolvePluginSourceRoots({
          env: {
            ...process.env,
            HOME: homeDir,
            SYNTHIOS_BUNDLED_PLUGINS_DIR: "~/bundled",
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".synthios", "extensions"),
    });
  });
});
