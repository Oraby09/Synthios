...
export type SynthiosPluginConfigSchema = {
...
export type SynthiosPluginToolContext = {
...
export type SynthiosPluginToolFactory = (
  ctx: SynthiosPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type SynthiosPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type SynthiosPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<SynthiosConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Synthios uses this to expose
 ...
 *
 * Runs after Synthios resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Synthios auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Synthios wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Synthios builds its generic
...
/**
 * Provider-owned stream wrapper hook after Synthios applies its generic
 ...
 *
 * Return `true` or `false` to override Synthios's built-in provider cache TTL
 ...
 *
 * Runs only after Synthios exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after Synthios loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * Synthios calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Synthios wrappers.
   *
   ...
   *
   * Called after Synthios resolves the raw configured credential but before the
   ...
   * hint than Synthios's generic auth-store guidance.
   */
   ...
   * `errorMessage` when Synthios should surface a provider-specific hint for
   ...
   * merging. Synthios deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Synthios uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `synthios doctor`.
   *
   ...
   *
   * Synthios calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Synthios's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: SynthiosConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: SynthiosConfig, value: unknown) => void;
  applySelectionConfig?: (config: SynthiosConfig) => SynthiosConfig;
...
export type SynthiosPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current Synthios configuration */
  config: SynthiosConfig;
 ...
 */
export type SynthiosPluginCommandDefinition = {
...
export type SynthiosPluginHttpRouteAuth = "gateway" | "plugin";
export type SynthiosPluginHttpRouteMatch = "exact" | "prefix";

export type SynthiosPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type SynthiosPluginHttpRouteParams = {
  path: string;
  handler: SynthiosPluginHttpRouteHandler;
  auth: SynthiosPluginHttpRouteAuth;
  match?: SynthiosPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type SynthiosPluginCliContext = {
  program: Command;
  config: SynthiosConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type SynthiosPluginCliRegistrar = (ctx: SynthiosPluginCliContext) => void | Promise<void>;

export type SynthiosPluginServiceContext = {
  config: SynthiosConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type SynthiosPluginService = {
  id: string;
  start: (ctx: SynthiosPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: SynthiosPluginServiceContext) => void | Promise<void>;
};

export type SynthiosPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

export type SynthiosPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: SynthiosPluginApi) => void | Promise<void>;
  activate?: (api: SynthiosPluginApi) => void | Promise<void>;
};

export type SynthiosPluginModule =
  | SynthiosPluginDefinition
  | ((api: SynthiosPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

export type SynthiosPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | SynthiosPluginToolFactory,
    opts?: SynthiosPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: SynthiosPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: SynthiosPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: SynthiosPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: SynthiosPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: SynthiosPluginService) => void;
   ...
   */
  registerCommand: (command: SynthiosPluginCommandDefinition) => void;
...
export type PluginFormat = "synthios" | "bundle";
