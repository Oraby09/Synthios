...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.SYNTHIOS_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
  ...
  try {
    const packageRoots = [
      resolveSynthiosPackageRootSync({ cwd: process.cwd() }),
      resolveSynthiosPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
