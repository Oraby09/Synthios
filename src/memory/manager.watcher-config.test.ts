...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): SynthiosConfig {
    const defaults: NonNullable<NonNullable<SynthiosConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: SynthiosConfig) {
