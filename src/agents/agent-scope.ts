...
type AgentEntry = NonNullable<NonNullable<SynthiosConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: SynthiosConfig): AgentEntry[] {
...
export function listAgentIds(cfg: SynthiosConfig): string[] {
...
export function resolveDefaultAgentId(cfg: SynthiosConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: SynthiosConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: SynthiosConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: SynthiosConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
