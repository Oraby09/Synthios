...
function findTool(name: string, config: SynthiosConfig) {
  const allTools = createSynthiosTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: SynthiosConfig; env?: NodeJS.ProcessEnv }) {
...
describe("synthios tools runtime web metadata wiring", () => {
