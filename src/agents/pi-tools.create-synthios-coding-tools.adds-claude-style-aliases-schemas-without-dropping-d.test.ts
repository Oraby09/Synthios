...
describe("createSynthiosCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-read-"));
    try {
      ...
      const contents = "Hello from synthios read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createSynthiosCodingTools({ sandbox });
