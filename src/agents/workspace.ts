...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.SYNTHIOS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".synthios", `workspace-${profile}`);
  }
  return path.join(home, ".synthios", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".synthios";
