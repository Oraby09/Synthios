...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SYNTHIOS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SYNTHIOS_HOME: "/srv/synthios-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/synthios-home"), ".synthios", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".synthios", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("synthios-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("synthios-workspace-");
    await fs.mkdir(path.join(tempDir, ".synthios"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-workspace-hardlink-"));
    try {
