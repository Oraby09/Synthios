...
const defaultTools = createSynthiosCodingTools({ senderIsOwner: true });
...
describe("createSynthiosCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: SynthiosConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createSynthiosCodingTools({
    ...
    const anthropicTools = createSynthiosCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: SynthiosConfig = {
      tools: {
        exec: {
    ...
    const allowed = createSynthiosCodingTools({
    ...
    const denied = createSynthiosCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createSynthiosCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
