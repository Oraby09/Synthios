...
function resolveAttachmentLimits(config: SynthiosConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".synthios", "attachments");
  const relDir = path.posix.join(".synthios", "attachments", attachmentId);
