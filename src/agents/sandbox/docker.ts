...
import { markSynthiosExecEnv } from "../../infra/synthios-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "synthios.sandbox=1");
  args.push("--label", `synthios.sessionKey=${params.scopeKey}`);
  args.push("--label", `synthios.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `synthios.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markSynthiosExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "synthios.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`synthios sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`synthios sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("synthios sandbox recreate --all");
}
