...
let previousConfigPath = process.env.SYNTHIOS_CONFIG_PATH;
...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(() => {
    ...
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `synthios-subagent-attachments-${process.pid}-${Date.now()}-`),
    );
    configPathOverride = path.join(workspaceDirOverride, "synthios.test.json");
    fs.writeFileSync(configPathOverride, JSON.stringify(configOverride, null, 2));
    previousConfigPath = process.env.SYNTHIOS_CONFIG_PATH;
    process.env.SYNTHIOS_CONFIG_PATH = configPathOverride;
  });
  ...
  afterEach(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.SYNTHIOS_CONFIG_PATH = previousConfigPath;
    }
    configPathOverride = "";
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".synthios", "attachments");
