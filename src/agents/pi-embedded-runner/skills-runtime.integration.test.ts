...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("synthios-bundled-");
  const workspaceDir = await createTempDir("synthios-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: SynthiosConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.SYNTHIOS_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
