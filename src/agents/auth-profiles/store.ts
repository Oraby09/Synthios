 ...
 * A common mistake — caused by the similarity with the `synthios.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (synthios.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.SYNTHIOS_AUTH_STORE_READONLY === "1";
