...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SynthiosConfig) {
    return createSynthiosCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SynthiosConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SynthiosConfig["agents"]>["list"]>[number]["tools"];
  }): SynthiosConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createSynthiosCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createSynthiosCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createSynthiosCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createSynthiosCodingTools({
      config: cfg,
    ...
    const bobTools = createSynthiosCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createSynthiosCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createSynthiosCodingTools({
      config: cfg,
