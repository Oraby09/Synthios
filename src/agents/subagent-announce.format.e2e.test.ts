...
let configOverride: SynthiosConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: SynthiosConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/synthios/synthios/issues/31298
    previousFastTestEnv = process.env.SYNTHIOS_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.SYNTHIOS_TEST_FAST;
      return;
    }
    process.env.SYNTHIOS_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // SYNTHIOS_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Synthios runtime context (internal):");
