...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseSynthiosManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applySynthiosManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): SynthiosSkillMetadata | undefined {
