...
type CoreToolDefinition = {
  ...
  includeInSynthiosGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const synthiosTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInSynthiosGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:synthios": synthiosTools,
