...
} from "./synthios-tools.subagents.sessions-spawn.test-harness.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.SYNTHIOS_TEST_FAST;
...
describe("synthios-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    ...
    process.env.SYNTHIOS_TEST_FAST = fastModeEnv.previous;
  });
