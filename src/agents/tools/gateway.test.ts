...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    synthios: process.env.SYNTHIOS_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.synthios === undefined) {
      delete process.env.SYNTHIOS_GATEWAY_TOKEN;
    } else {
      process.env.SYNTHIOS_GATEWAY_TOKEN = envSnapshot.synthios;
    }
  ...
  it("uses SYNTHIOS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SYNTHIOS_GATEWAY_TOKEN = "local-env-token";
