...
import { planSynthiosModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: SynthiosConfig): {
  config: SynthiosConfig;
  sourceConfigForSecrets: SynthiosConfig;
} {
...
export async function ensureSynthiosModelsJson(
  config?: SynthiosConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveSynthiosAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planSynthiosModelsJson({
      cfg,
