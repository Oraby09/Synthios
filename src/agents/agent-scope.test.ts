...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: SynthiosConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/synthios" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: SynthiosConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: SynthiosConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: SynthiosConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: SynthiosConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: SynthiosConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: SynthiosConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: SynthiosConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/synthios");
  });

  it("uses SYNTHIOS_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "synthios-home");
    vi.stubEnv("SYNTHIOS_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as SynthiosConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".synthios", "workspace"));
  });

  it("uses SYNTHIOS_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "synthios-home");
    vi.stubEnv("SYNTHIOS_HOME", home);
    // Clear state dir so it falls back to SYNTHIOS_HOME
    vi.stubEnv("SYNTHIOS_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as SynthiosConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".synthios", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/synthios-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/synthios-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-agent-scope-"));
