...
import { resolvePreferredSynthiosTmpDir } from "../infra/tmp-synthios-dir.js";
...
async function withOutsideHardlinkInSynthiosTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside synthios tmp root",
      media: path.join(os.tmpdir(), "outside-synthios", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Synthios tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Synthios tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSynthiosTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Synthios tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSynthiosTmp(
