...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["SYNTHIOS_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-subagent-"));
  ...
  it("uses isolated temp state when SYNTHIOS_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "synthios-test-state"));
  });
});
