...
const GOOGLE_LIVE_SINGLE_KEY = "SYNTHIOS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SYNTHIOS_LIVE_ANTHROPIC_KEY",
    listVar: "SYNTHIOS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SYNTHIOS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SYNTHIOS_LIVE_${base}_KEY`;
