...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("synthios-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("synthios-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("synthios-lid-a-", async (first) => {
      await withTempDir("synthios-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.synthios when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "synthios-config-dir-"));
    try {
  ...
  it("expands SYNTHIOS_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/synthios-home",
      SYNTHIOS_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/synthios-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers SYNTHIOS_HOME over HOME", () => {
    vi.stubEnv("SYNTHIOS_HOME", "/srv/synthios-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/synthios-home"));
...
describe("shortenHomePath", () => {
  it("uses $SYNTHIOS_HOME prefix when SYNTHIOS_HOME is set", () => {
    vi.stubEnv("SYNTHIOS_HOME", "/srv/synthios-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/synthios-home")}/.synthios/synthios.json`)).toBe(
      "$SYNTHIOS_HOME/.synthios/synthios.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $SYNTHIOS_HOME replacement when SYNTHIOS_HOME is set", () => {
    vi.stubEnv("SYNTHIOS_HOME", "/srv/synthios-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/synthios-home")}/.synthios/synthios.json`),
    ).toBe("config: $SYNTHIOS_HOME/.synthios/synthios.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/synthios", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "synthios"),
    );
  });
  ...
  it("prefers SYNTHIOS_HOME for tilde expansion", () => {
    vi.stubEnv("SYNTHIOS_HOME", "/srv/synthios-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/synthios")).toBe(path.resolve("/srv/synthios-home", "synthios"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/synthios", env)).toBe(path.resolve("/srv/synthios-home", "synthios"));
  });
