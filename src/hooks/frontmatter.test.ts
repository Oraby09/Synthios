...
`;
    ...
    expect(parsed.synthios.emoji).toBe("💾");
    expect(parsed.synthios.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.synthios.emoji).toBe("📝");
    expect(parsed.synthios.events).toEqual(["command"]);
    expect(parsed.synthios.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.synthios.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"synthios": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"synthios": {"events": ["test"]}}');
  });
...
describe("resolveSynthiosMetadata", () => {
  it("extracts synthios metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSynthiosMetadata(frontmatter);
  ...
  it("returns undefined when synthios key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        synthios: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Synthios" },
            { id: "npm", kind: "npm", package: "@synthios/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@synthios/hook");
  });
    ...
    "synthios":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Synthios" }],
      },
  }
---
...
`;
    ...
    const synthios = resolveSynthiosMetadata(frontmatter);
    expect(synthios).toBeDefined();
    expect(synthios?.emoji).toBe("💾");
    expect(synthios?.events).toEqual(["command:new", "command:reset"]);
    expect(synthios?.requires?.config).toEqual(["workspace.dir"]);
    expect(synthios?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const synthios = resolveSynthiosMetadata(frontmatter);
    expect(synthios?.emoji).toBe("disk");
    expect(synthios?.events).toEqual(["command:new"]);
  });
});
