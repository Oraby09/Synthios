...
homepage: https://docs.synthios.ai/automation/hooks#command-logger
metadata:
  {
    "synthios":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.synthios/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.synthios/logs/commands.log ~/.synthios/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/synthios`:
   ```
   /home/username/.synthios/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
