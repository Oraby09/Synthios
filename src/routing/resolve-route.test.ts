...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: SynthiosConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: SynthiosConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/synthios-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<SynthiosConfig["bindings"]>[number];
