...
function listAgents(cfg: SynthiosConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: SynthiosConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<SynthiosConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: SynthiosConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: SynthiosConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SynthiosConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<SynthiosConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  SynthiosConfig,
  {
    bindingsRef: SynthiosConfig["bindings"];
    agentsRef: SynthiosConfig["agents"];
    sessionRef: SynthiosConfig["session"];
...
function resolveRouteCacheForConfig(cfg: SynthiosConfig): Map<string, ResolvedAgentRoute> {
