...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-install-sh-"));
  const cliPath = path.join(root, "synthios");
...
SYNTHIOS_BIN="$FAKE_SYNTHIOS_BIN"
resolve_synthios_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_SYNTHIOS_BIN: cliPath,
        SYNTHIOS_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Synthios 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Synthios dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Synthios dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-install-stdin-"));
