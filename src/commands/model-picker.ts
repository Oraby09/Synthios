...
type PromptDefaultModelResult = { model?: string; config?: SynthiosConfig };
...
function resolveConfiguredModelRaw(cfg: SynthiosConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: SynthiosConfig): string[] {
...
export function applyModelAllowlist(cfg: SynthiosConfig, models: string[]): SynthiosConfig {
