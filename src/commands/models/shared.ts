...
export async function loadValidConfigOrThrow(): Promise<SynthiosConfig> {
...
export async function updateConfig(
  mutator: (cfg: SynthiosConfig) => SynthiosConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SynthiosConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SynthiosConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("synthios agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
