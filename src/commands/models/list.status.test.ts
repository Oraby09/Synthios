...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSynthiosAgentDir: vi.fn().mockReturnValue("/tmp/synthios-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/synthios-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/synthios-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/synthios-dev/synthios.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSynthiosAgentDir: mocks.resolveSynthiosAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/synthios-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSynthiosAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/synthios-dev/synthios.json");
    expect(payload.auth.storePath).toBe("/tmp/synthios-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/synthios-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/synthios-agent-custom");
