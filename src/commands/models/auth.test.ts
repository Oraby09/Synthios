...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: SynthiosConfig;
  let lastUpdatedConfig: SynthiosConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/synthios/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/synthios/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/synthios/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: SynthiosConfig) => SynthiosConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/synthios/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/synthios/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `synthios plugins list --json`.',
    );
  });
