...
import { readConfigFileSnapshot, writeConfigFile, type SynthiosConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("synthios config set gateway.auth.mode token")} or ${formatCliCommand("synthios config set gateway.auth.mode password")}.`,
  ].join(" ");
}
