...
type EnvSnapshot = {
  HOME?: string;
  SYNTHIOS_HOME?: string;
  SYNTHIOS_STATE_DIR?: string;
  SYNTHIOS_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    SYNTHIOS_HOME: process.env.SYNTHIOS_HOME,
    SYNTHIOS_STATE_DIR: process.env.SYNTHIOS_STATE_DIR,
    SYNTHIOS_OAUTH_DIR: process.env.SYNTHIOS_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: SynthiosConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: SynthiosConfig) {
...
async function runStateIntegrityText(cfg: SynthiosConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.SYNTHIOS_HOME = tempHome;
    process.env.SYNTHIOS_STATE_DIR = path.join(tempHome, ".synthios");
    delete process.env.SYNTHIOS_OAUTH_DIR;
    fs.mkdirSync(process.env.SYNTHIOS_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when SYNTHIOS_OAUTH_DIR is explicitly configured", async () => {
    process.env.SYNTHIOS_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: SynthiosConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints synthios-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: SynthiosConfig = {};
    ...
    expect(text).toMatch(/synthios sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/synthios sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /synthios sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
