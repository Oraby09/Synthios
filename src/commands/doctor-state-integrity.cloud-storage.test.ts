...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Synthios", ".synthios");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/synthios-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".synthios");
  ...
  it("anchors cloud detection to OS homedir when SYNTHIOS_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".synthios");
    const originalSynthiosHome = process.env.SYNTHIOS_HOME;
    process.env.SYNTHIOS_HOME = "/tmp/synthios-home-override";
    ...
    try {
      ...
      if (originalSynthiosHome === undefined) {
        delete process.env.SYNTHIOS_HOME;
      } else {
        process.env.SYNTHIOS_HOME = originalSynthiosHome;
      }
    }
  });
