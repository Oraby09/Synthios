...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: SynthiosConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: SynthiosConfig) => { changed: boolean; next: SynthiosConfig };
  defaultModel: string;
  overrideConfig: SynthiosConfig;
  alreadyDefaultConfig: SynthiosConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: SynthiosConfig) => config,
      applyDefaultConfig: (config: SynthiosConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: SynthiosConfig = { agents: { defaults: {} } };
