...
function resolveAgentName(cfg: SynthiosConfig, agentId: string) {
...
function resolveAgentModel(cfg: SynthiosConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SynthiosConfig): AgentSummary[] {
