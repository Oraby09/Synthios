...
import { SynthiosSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: SynthiosConfig): {
  config: SynthiosConfig;
  removed: string[];
} {
  const parsed = SynthiosSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: SynthiosConfig): void {
