...
import type { SynthiosConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: SynthiosConfig): string[] {
...
export async function noteSecurityWarnings(cfg: SynthiosConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("synthios security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.synthios/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("synthios approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.synthios.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("synthios configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("synthios config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("synthios doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("synthios config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('synthios config set session.dmScope "per-channel-peer"') +
