...
export const resolveSynthiosPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadSynthiosPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveSynthiosPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SYNTHIOS_STATE_DIR;
  originalUpdateInProgress = process.env.SYNTHIOS_UPDATE_IN_PROGRESS;
  process.env.SYNTHIOS_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.SYNTHIOS_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
