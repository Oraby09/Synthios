...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under SYNTHIOS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("synthios-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-oauth-sync-"));
    ...
    process.env.SYNTHIOS_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-oauth-nosync-"));
    process.env.SYNTHIOS_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside SYNTHIOS_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-oauth-external-"));
    process.env.SYNTHIOS_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* SYNTHIOS_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SYNTHIOS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("synthios-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
