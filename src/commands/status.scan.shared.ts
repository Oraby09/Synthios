...
export function hasExplicitMemorySearchConfig(cfg: SynthiosConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: SynthiosConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: SynthiosConfig, agentId: string) => { store: { path: string } } | null;
