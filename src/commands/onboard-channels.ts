...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("synthios pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelSetupDmPolicy) => {
    ...
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`synthios pairing approve ${policy.channel} <code>`)}`,
    ...
    if (!plugin) {
      if (adapter) {
        await prompter.note(
          ...
          )}\` and \`${formatCliCommand("synthios plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
        await refreshStatus(channel);
        return true;
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("synthios channels add")}\``,
        },
      ],
