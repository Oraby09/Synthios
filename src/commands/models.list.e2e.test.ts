...
const ensureSynthiosModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveSynthiosAgentDir = vi.fn().mockReturnValue("/tmp/synthios-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureSynthiosModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveSynthiosAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureSynthiosModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureSynthiosModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureSynthiosModelsJson).toHaveBeenCalled();
    expect(ensureSynthiosModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
