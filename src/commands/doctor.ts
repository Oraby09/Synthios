...
import { resolveSynthiosPackageRoot } from "../infra/synthios-root.js";
...
function resolveMode(cfg: SynthiosConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Synthios doctor");

  const root = await resolveSynthiosPackageRoot({
  ...
  let cfg: SynthiosConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("synthios configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("synthios config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("synthios setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("synthios config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("synthios config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("synthios doctor --fix")}" to apply changes.`);
  }
