...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "SYNTHIOS_GATEWAY_TOKEN",
          placeholder: "SYNTHIOS_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like SYNTHIOS_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Synthios will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Synthios trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.synthios.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
