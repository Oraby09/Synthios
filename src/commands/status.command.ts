...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedBySynthios ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedBySynthios ? "installed · " : "";
  ...
  runtime.log(theme.heading("Synthios status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`synthios devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("synthios devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("synthios devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("synthios security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("synthios security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.synthios.ai/faq");
  runtime.log("Troubleshooting: https://docs.synthios.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("synthios status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("synthios logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("synthios status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("synthios gateway probe")}`);
  }
}
