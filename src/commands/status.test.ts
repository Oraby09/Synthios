...
beforeAll(() => {
  envSnapshot = captureEnv(["SYNTHIOS_PROFILE"]);
  process.env.SYNTHIOS_PROFILE = "isolated";
});
...
vi.mock("../infra/synthios-root.js", () => ({
  resolveSynthiosPackageRoot: vi.fn().mockResolvedValue("/tmp/synthios"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/synthios/pnpm-lock.yaml",
      markerPath: "/tmp/synthios/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.synthios.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.synthios.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("synthios status --all") ||
          line.includes("synthios --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("SYNTHIOS_GATEWAY_TOKEN", "abcd1234", async () => {
