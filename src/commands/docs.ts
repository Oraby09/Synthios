...
const SEARCH_TOOL = "https://docs.synthios.ai/mcp.SearchSynthios";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.synthios.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('synthios docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.synthios.ai/");
      runtime.log(`Search: ${formatCliCommand('synthios docs "your query"')}`);
    }
    return;
  }
