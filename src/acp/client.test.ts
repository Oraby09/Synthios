...
const createTempDir = () => tempDirs.make("synthios-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets SYNTHIOS_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "synthios",
    });

    expect(env.SYNTHIOS_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("synthios");
  });

  it("overrides pre-existing SYNTHIOS_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      SYNTHIOS_SHELL: "wrong",
    });
    expect(env.SYNTHIOS_SHELL).toBe("acp-client");
  });
  ...
  it("preserves SYNTHIOS_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        SYNTHIOS_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["SYNTHIOS_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Synthios bridge", () => {
    ...
    expect(env.SYNTHIOS_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.SYNTHIOS_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "synthios",
        serverArgs: ["acp"],
        defaultServerCommand: "synthios",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("SYNTHIOS_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "synthios", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "synthios", "dist", "entry.js");
    const shimPath = path.join(dir, "synthios.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\synthios\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/synthios-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/synthios-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/synthios-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
