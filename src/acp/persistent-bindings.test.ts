...
type ConfiguredBinding = NonNullable<SynthiosConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<SynthiosConfig>,
): SynthiosConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: SynthiosConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/synthios" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
