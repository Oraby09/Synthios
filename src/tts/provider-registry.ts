...
function resolveSpeechProviderPluginEntries(cfg?: SynthiosConfig): SpeechProviderPlugin[] {
  const active = getActivePluginRegistry();
  const registry =
    (active?.speechProviders?.length ?? 0) > 0 || !cfg
      ? active
      : loadSynthiosPlugins({ config: cfg });
  return registry?.speechProviders?.map((entry) => entry.provider) ?? [];
}

function buildProviderMaps(cfg?: SynthiosConfig): {
...
export function listSpeechProviders(cfg?: SynthiosConfig): SpeechProviderPlugin[] {
