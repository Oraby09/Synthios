...
export function resolveTtsConfig(cfg: SynthiosConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.SYNTHIOS_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: SynthiosConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: SynthiosConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      const tempRoot = resolvePreferredSynthiosTmpDir();
