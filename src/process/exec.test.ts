...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        SYNTHIOS_BASE_ENV: "base",
        SYNTHIOS_TO_REMOVE: undefined,
      },
      env: {
        SYNTHIOS_TEST_ENV: "ok",
      },
    });

    expect(resolved.SYNTHIOS_BASE_ENV).toBe("base");
    expect(resolved.SYNTHIOS_TEST_ENV).toBe("ok");
    expect(resolved.SYNTHIOS_TO_REMOVE).toBeUndefined();
    expect(resolved.SYNTHIOS_CLI).toBe(SYNTHIOS_CLI_ENV_VALUE);
  });
