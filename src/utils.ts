...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".synthios");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SYNTHIOS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SYNTHIOS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SYNTHIOS_STATE_DIR.
