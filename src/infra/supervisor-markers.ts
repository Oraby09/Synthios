const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "SYNTHIOS_LAUNCHD_LABEL"],
  systemd: ["SYNTHIOS_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["SYNTHIOS_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.SYNTHIOS_SERVICE_MARKER?.trim();
    const serviceKind = env.SYNTHIOS_SERVICE_KIND?.trim();
