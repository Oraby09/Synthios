...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.SYNTHIOS_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Synthios. You can keep running the web gateway without it: \`${formatCliCommand("synthios gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
