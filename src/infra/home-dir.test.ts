...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers SYNTHIOS_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for SYNTHIOS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { SYNTHIOS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when SYNTHIOS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ SYNTHIOS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("resolveOsHomeDir", () => {
  it("ignores SYNTHIOS_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          SYNTHIOS_HOME: "/srv/synthios-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { SYNTHIOS_HOME: "/srv/synthios-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/synthios-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/synthios-home " },
      expected: "/srv/synthios-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/synthios-home/docs"));
  });
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of SYNTHIOS_HOME", () => {
    expect(
