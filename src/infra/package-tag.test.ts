...
describe("normalizePackageTagInput", () => {
  const packageNames = ["synthios", "@synthios/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("synthios@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@synthios/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("synthios@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("synthios", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @synthios/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("synthioser@beta", packageNames)).toBe("synthioser@beta");
  });
});
