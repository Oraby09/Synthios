...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SynthiosConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: SynthiosConfig) {
...
function resolveHeartbeatAgents(cfg: SynthiosConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SynthiosConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SynthiosConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SynthiosConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
