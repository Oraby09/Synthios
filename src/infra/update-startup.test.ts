...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SYNTHIOS_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.SYNTHIOS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveSynthiosPackageRoot } = await import("./synthios-root.js"));
    ...
    vi.mocked(resolveSynthiosPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveSynthiosPackageRoot).mockResolvedValue("/opt/synthios");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/synthios/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
