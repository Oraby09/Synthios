...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SYNTHIOS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*synthios-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "synthios";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "synthios");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "synthios-gw");
  ...
  records.push(`_synthios-gw._tcp IN PTR ${instanceLabel}._synthios-gw._tcp`);
  records.push(`${instanceLabel}._synthios-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._synthios-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; synthios-content-hash: ${contentHash}\n${contentBody}`;
}
