...
const DEFAULT_PACKAGE_NAME = "synthios";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["synthios", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "synthios.mjs");
    ...
    const doctorStep = await runStep(
      step("synthios doctor", doctorArgv, gitRoot, { SYNTHIOS_UPDATE_IN_PROGRESS: "1" }),
    );
