...
const PRIMARY_PACKAGE_NAME = "synthios";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const SYNTHIOS_MAIN_PACKAGE_SPEC = "github:synthios/synthios#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Synthios", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.SYNTHIOS_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.SYNTHIOS_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return SYNTHIOS_MAIN_PACKAGE_SPEC;
  }
