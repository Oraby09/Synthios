...
export function triggerSynthiosRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.SYNTHIOS_SYSTEMD_UNIT,
      process.env.SYNTHIOS_PROFILE,
    );
  ...
  const label =
    process.env.SYNTHIOS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SYNTHIOS_PROFILE);
