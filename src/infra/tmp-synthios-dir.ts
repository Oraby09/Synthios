...
export const POSIX_SYNTHIOS_TMP_DIR = "/tmp/synthios";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredSynthiosTmpDirOptions = {
...
export function resolvePreferredSynthiosTmpDir(
  options: ResolvePreferredSynthiosTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "synthios" : `synthios-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[synthios] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Synthios temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Synthios temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_SYNTHIOS_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_SYNTHIOS_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_SYNTHIOS_TMP_DIR)) {
      return POSIX_SYNTHIOS_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_SYNTHIOS_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_SYNTHIOS_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_SYNTHIOS_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_SYNTHIOS_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_SYNTHIOS_TMP_DIR;
