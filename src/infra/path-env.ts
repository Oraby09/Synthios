...
function candidateBinDirs(opts: EnsureSynthiosPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `synthios` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "synthios");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SYNTHIOS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "synthios"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `synthios` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSynthiosCliOnPath(opts: EnsureSynthiosPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SYNTHIOS_PATH_BOOTSTRAPPED)) {
    return;
  }
