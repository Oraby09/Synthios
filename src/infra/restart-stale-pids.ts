 ...
 * returning control to the caller (typically `triggerSynthiosRestart` →
...
/**
 * Parse synthios gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("synthios")) {
 ...
 * Returns only PIDs that belong to synthios gateway processes (not the current process).
 */
