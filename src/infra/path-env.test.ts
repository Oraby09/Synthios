...
let ensureSynthiosCliOnPath: typeof import("./path-env.js").ensureSynthiosCliOnPath;

describe("ensureSynthiosCliOnPath", () => {
  const envKeys = [
    "PATH",
    "SYNTHIOS_PATH_BOOTSTRAPPED",
    "SYNTHIOS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureSynthiosCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/synthios-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "synthios");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureSynthiosCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling synthios exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.SYNTHIOS_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.SYNTHIOS_PATH_BOOTSTRAPPED = "1";
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "synthios");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.SYNTHIOS_PATH_BOOTSTRAPPED;
      delete process.env.SYNTHIOS_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.SYNTHIOS_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/synthios-path/case-linuxbrew");
