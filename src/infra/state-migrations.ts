...
import { listTelegramAccountIds } from "synthios/plugin-sdk/telegram";
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set SYNTHIOS_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.SYNTHIOS_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
