...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SynthiosConfig | null = null;
  ...
  get(key: string, cfg: SynthiosConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SynthiosConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: SynthiosConfig): void {
