...
const loadSynthiosPluginsMock = vi.hoisted(() => vi.fn());
...
vi.mock("../../plugins/loader.js", () => ({
  loadSynthiosPlugins: (...args: unknown[]) => loadSynthiosPluginsMock(...args),
}));
...
describe("outbound channel resolution", () => {
  beforeEach(() => {
    ...
    loadSynthiosPluginsMock.mockReset();
  ...
  it("returns the already-registered plugin without bootstrapping", async () => {
    ...
    expect(loadSynthiosPluginsMock).not.toHaveBeenCalled();
  });
  ...
  it("bootstraps plugins once per registry key and returns the newly loaded plugin", async () => {
    ...
    expect(loadSynthiosPluginsMock).toHaveBeenCalledWith({
    ...
    expect(loadSynthiosPluginsMock).toHaveBeenCalledTimes(1);
    expect(loadSynthiosPluginsMock).toHaveBeenLastCalledWith({
      config: { autoEnabled: true },
      workspaceDir: "/tmp/workspace",
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
  ...
  it("retries bootstrap after a transient load failure", async () => {
    getChannelPluginMock.mockReturnValue(undefined);
    loadSynthiosPluginsMock.mockImplementationOnce(() => {
    ...
    expect(loadSynthiosPluginsMock).toHaveBeenCalledTimes(2);
  });
});
