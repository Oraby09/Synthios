...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Synthios tmp root", async () => {
      const tmpRoot = resolvePreferredSynthiosTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideSynthios = path.join(os.tmpdir(), "outside-synthios", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
