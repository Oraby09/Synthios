...
const { makeStorePath } = createCronStoreHarness({ prefix: "synthios-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses SYNTHIOS_HOME for tilde expansion", () => {
    vi.stubEnv("SYNTHIOS_HOME", "/srv/synthios-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/synthios-home", "cron", "jobs.json"));
  });
});
