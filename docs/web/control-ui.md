...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/synthios`)
...
If the page fails to load, start the Gateway first: `synthios gateway`.
...
you revoke it with `synthios devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.synthios/synthios.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop synthios`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Synthios
...
Synthios **blocks** Control UI connections without device identity.
