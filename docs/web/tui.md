...
```bash
synthios tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `synthios tui --deliver`
...
- Local shell commands receive `SYNTHIOS_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `SYNTHIOS_THEME=light` before launching `synthios tui`.
- To force the original dark palette instead, set `SYNTHIOS_THEME=dark`.
...
- Check the Gateway logs: `synthios logs --follow`.
- Confirm the agent can run: `synthios status` and `synthios models status`.
...
- No agents in picker: check `synthios agents list` and your routing config.
