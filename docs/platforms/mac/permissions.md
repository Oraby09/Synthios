...
- Same path: run the app from a fixed location (for Synthios, `dist/Synthios.app`).
...
```bash
sudo tccutil reset Accessibility ai.synthios.mac
sudo tccutil reset ScreenCapture ai.synthios.mac
...
Workaround: move files into the Synthios workspace (`~/.synthios/workspace`) if you want to avoid per-folder grants.
