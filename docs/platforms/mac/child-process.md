...
service via the external `synthios` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.synthios.gateway`
  (or `ai.synthios.<profile>` when using `--profile`/`SYNTHIOS_PROFILE`; legacy `com.synthios.*` is supported).
