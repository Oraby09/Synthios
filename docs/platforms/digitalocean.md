---
summary: "Synthios on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Synthios on DigitalOcean
  - Looking for cheap VPS hosting for Synthios
title: "DigitalOcean"
---

# Synthios on DigitalOcean

## Goal

Run a persistent Synthios Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status synthios-gateway.service

# View logs
journalctl --user -u synthios-gateway.service -f
```
...
```bash
synthios config set gateway.bind tailnet
synthios gateway restart
```
...
```bash
synthios channels login whatsapp
# Scan QR code
```
...
- `~/.synthios/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u synthios --no-pager -n 50
```
