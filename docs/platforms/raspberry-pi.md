---
summary: "Synthios on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Synthios on a Raspberry Pi
  - Running Synthios on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Synthios on Raspberry Pi

## Goal

Run a persistent, always-on Synthios Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u synthios -f
```

## 9) Access the Synthios Dashboard
...
```bash
ssh user@gateway-host 'synthios dashboard --no-open'
```
...
(or `SYNTHIOS_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/synthios-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/synthios-compile-cache
mkdir -p /var/tmp/synthios-compile-cache
export SYNTHIOS_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `SYNTHIOS_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Synthios, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit synthios
```

```ini
[Service]
Environment=SYNTHIOS_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/synthios-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Synthios state/cache on SSD-backed storage to avoid SD-card
...
Most Synthios features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled synthios

# Enable if not
sudo systemctl enable synthios

# Start on boot
sudo systemctl start synthios
```
...
```bash
# Check logs
journalctl -u synthios --no-pager -n 100

# Common fix: rebuild
cd ~/synthios  # if using hackable install
npm run build
sudo systemctl restart synthios
```
