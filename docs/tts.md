...
Synthios can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Synthios can send audio; Telegram gets a round voice-note bubble.
...
Synthios defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `synthios.json`.
...
- If `provider` is **unset**, Synthios prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.synthios/settings/tts.json`, override with `SYNTHIOS_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Synthios retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, Synthios:
...
Discord note: `/tts` is a built-in Discord command, so Synthios registers
