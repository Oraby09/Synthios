---
summary: "Frequently asked questions about Synthios setup, configuration, and usage"
read_when:
...
- [Quick start and first-run setup]
  ...
  - [Recommended way to install and set up Synthios](#recommended-way-to-install-and-set-up-synthios)
  ...
  - [Cannot access docs.synthios.ai (SSL error)](#cannot-access-docssynthiosai-ssl-error)
  ...
  - [Windows install says git not found or synthios not recognized](#windows-install-says-git-not-found-or-synthios-not-recognized)
  ...
  - [How do I install Synthios on Linux?](#how-do-i-install-synthios-on-linux)
  - [How do I install Synthios on a VPS?](#how-do-i-install-synthios-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask Synthios to update itself?](#can-i-ask-synthios-to-update-itself)
  ...
  - [If I buy a Mac mini to run Synthios, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-synthios-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different Synthios instances?](#can-multiple-people-use-one-whatsapp-number-with-different-synthios-instances)
  ...
  - [How important is it to run Synthios on a dedicated machine?](#how-important-is-it-to-run-synthios-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run Synthios in a VM and what are the requirements](#can-i-run-synthios-in-a-vm-and-what-are-the-requirements)
- [What is Synthios?](#what-is-synthios)
  - [What is Synthios, in one paragraph?](#what-is-synthios-in-one-paragraph)
  - [Value proposition](#value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for Synthios](#what-are-the-top-five-everyday-use-cases-for-synthios)
  - [Can Synthios help with lead gen outreach ads and blogs for a SaaS](#can-synthios-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can Synthios run tasks on a schedule or continuously in the background?](#can-synthios-run-tasks-on-a-schedule-or-continuously-in-the-background)
  - [Can I run Apple macOS-only skills from Linux?](#can-i-run-apple-macos-only-skills-from-linux)
  - [Do you have a Notion or HeyGen integration?](#do-you-have-a-notion-or-heygen-integration)
  - [How do I use my existing signed-in Chrome with Synthios?](#how-do-i-use-my-existing-signed-in-chrome-with-synthios)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with Synthios saved locally?](#is-all-data-used-with-synthios-saved-locally)
  - [Where does Synthios store its data?](#where-does-synthios-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [Recommended backup strategy](#recommended-backup-strategy)
  - [How do I completely uninstall Synthios?](#how-do-i-completely-uninstall-synthios)
...
- [Config basics](#config-basics)
  ...
  - [Can the Synthios browser run headless?](#can-the-synthios-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two Synthios instances talk to each other (local + VPS)?](#can-two-synthios-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does Synthios load environment variables?](#how-does-synthios-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of Synthios instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-synthios-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset Synthios but keep it installed?](#how-do-i-completely-reset-synthios-but-keep-it-installed)
  ...
  - [Why does Synthios not reply in a group](#why-does-synthios-not-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do Synthios, Flawd, and Krill use for models?](#what-do-synthios-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `synthios gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-synthios-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `synthios gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-synthios-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run Synthios in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-synthios-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart Synthios?](#i-closed-my-terminal-on-windows-how-do-i-restart-synthios)
  ...
  - [ELI5: `synthios gateway restart` vs `synthios gateway`](#eli5-synthios-gateway-restart-vs-synthios-gateway)
...
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose Synthios to inbound DMs?](#is-it-safe-to-expose-synthios-to-inbound-dms)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/synthios/synthios-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   synthios health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://synthios.ai/install.sh | bash -s -- --install-method git
```

This installs Synthios **from a git checkout**, so the agent can read the code + docs and
...
[https://github.com/synthios/synthios/issues](https://github.com/synthios/synthios/issues)
[https://github.com/synthios/synthios/pulls](https://github.com/synthios/synthios/pulls)
...
- `synthios status`: quick snapshot of gateway/agent health + basic config.
- `synthios models status`: checks provider auth + model availability.
- `synthios doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `synthios status --all`, `synthios logs --follow`,
`synthios gateway status`, `synthios health --verbose`.

Quick debug loop: [First 60 seconds if something is broken](#first-60-seconds-if-something-is-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### Recommended way to install and set up Synthios

The repo recommends running from source and using onboarding:

```bash
curl -fsSL https://synthios.ai/install.sh | bash
synthios onboard --install-daemon
```
...
```bash
git clone https://github.com/synthios/synthios.git
cd synthios
pnpm install
pnpm build
...
If you don't have a global install yet, run it via `pnpm synthios onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `SYNTHIOS_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `synthios config get gateway.auth.token` (or generate one: `synthios doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `synthios gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `synthios gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install Synthios on the new machine.
2. Copy `$SYNTHIOS_STATE_DIR` (default: `~/.synthios`) from the old machine.
3. Copy your workspace (default: `~/.synthios/workspace`).
4. Run `synthios doctor` and restart the Gateway service.
...
under `~/.synthios/` (for example `~/.synthios/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-synthios-store-its-data),
...
[https://github.com/synthios/synthios/blob/main/CHANGELOG.md](https://github.com/synthios/synthios/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### Cannot access docs.synthios.ai (SSL error)

Some Comcast/Xfinity connections incorrectly block `docs.synthios.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.synthios.ai`, then retry. More
detail: [Troubleshooting](/help/faq#docssynthiosai-shows-an-ssl-error-comcast-xfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/synthios/synthios/tree/main/docs](https://github.com/synthios/synthios/tree/main/docs)
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://synthios.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://synthios.ai/install.ps1](https://synthios.ai/install.ps1)
...
```bash
curl -fsSL https://synthios.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://synthios.ai/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://synthios.ai/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://synthios.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or synthios not recognized
...
**2) synthios is not recognized after install**
...
If you still reproduce this on latest Synthios, track/report it in:

- [Issue #30640](https://github.com/synthios/synthios/issues/30640)
...
### How do I install Synthios on Linux
...
### How do I install Synthios on a VPS
...
### Can I ask Synthios to update itself
...
```bash
synthios update
synthios update status
synthios update --channel stable|beta|dev
synthios update --tag <dist-tag|version>
synthios update --no-restart
```

If you must automate from an agent:

```bash
synthios update --yes --no-restart
synthios gateway restart
```
...
`synthios onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run Synthios with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like Synthios.
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in onboarding. If you want to run it on the gateway host, use `synthios models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `synthios models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. Synthios no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so Synthios can keep replying while a provider is rate-limited.
...
Synthios supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. Synthios fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like Synthios. Onboarding can run the OAuth flow for you.

See [OAuth](/concepts/oauth), [Model providers](/concepts/model-providers), and [Onboarding (CLI)](/start/wizard).

### How do I set up Gemini CLI OAuth

Gemini CLI uses a **plugin auth flow**, not a client id or secret in `synthios.json`.

Steps:

1. Enable the plugin: `synthios plugins enable google`
2. Login: `synthios models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. Synthios needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. Synthios runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run Synthios can I connect it to my MacBook Pro
...
- Use `synthios nodes status` / `synthios nodes list` to see it.
...
Onboarding accepts `@username` input and resolves it to a numeric ID, but Synthios authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `synthios logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different Synthios instances
...
If you run Synthios via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the Synthios code install. Your state
(`~/.synthios`) and workspace (`~/.synthios/workspace`) stay untouched.
...
**Synthios-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run Synthios on a dedicated machine
...
Synthios is lightweight. For a basic Gateway + one chat channel:
...
### Can I run Synthios in a VM and what are the requirements
...
### What is Synthios in one paragraph

Synthios is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### Value proposition

Synthios is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for Synthios
...
### Can Synthios help with lead gen outreach ads and blogs for a SaaS
...
Synthios draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

Synthios is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use Synthios when you
...
Showcase: [https://synthios.ai/showcase](https://synthios.ai/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.synthios/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.synthios/synthios.json`). Precedence is `<workspace>/skills` > `~/.synthios/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.synthios/synthios.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.synthios/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Synthios treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `SYNTHIOS_SKIP_CRON` is not set.
...
```bash
synthios cron run <jobId> --force
synthios cron runs --id <jobId> --limit 50
```
...
### Can Synthios run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.synthios.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-synthios-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Synthios can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.synthios/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "synthios": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured Synthios workspace); Synthios treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.synthios/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).

### How do I use my existing signed-in Chrome with Synthios

Use the built-in `user` browser profile, which attaches through Chrome DevTools MCP:

```bash
synthios browser --browser-profile user tabs
synthios browser --browser-profile user snapshot
```

If you want a custom name, create an explicit MCP profile:

```bash
synthios browser create-profile --name chrome-live --driver existing-session
synthios browser --browser-profile chrome-live tabs
```
...
- Persist `/home/node` with `SYNTHIOS_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `SYNTHIOS_DOCKER_APT_PACKAGES`.
...
Synthios memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

Synthios also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, Synthios auto-selects a provider when it
...
configured and present, Synthios
...
### Is all data used with Synthios saved locally

No - **Synthios's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.synthios` + your workspace directory).
...
### Where does Synthios store its data

Everything lives under `$SYNTHIOS_STATE_DIR` (default: `~/.synthios`):

| Path                                                            | Purpose                                                            |
| --------------------------------------------------------------- | ------------------------------------------------------------------ |
| `$SYNTHIOS_STATE_DIR/synthios.json`                             | Main config (JSON5)                                                |
| `$SYNTHIOS_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
| `$SYNTHIOS_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
| `$SYNTHIOS_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
| `$SYNTHIOS_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
| `$SYNTHIOS_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
| `$SYNTHIOS_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
| `$SYNTHIOS_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
| `$SYNTHIOS_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

Legacy single-agent path: `~/.synthios/agent/*` (migrated by `synthios doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.synthios/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.synthios`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
  `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.synthios`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.synthios/skills`).

Default workspace is `~/.synthios/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.synthios/workspace" } },
}
```
...
Do **not** commit anything under `~/.synthios` (credentials, sessions, tokens, or encrypted secrets payloads).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall Synthios
...
`workspace` to the repo root. The Synthios repo is just source code; keep the
...
Synthios reads an optional **JSON5** config from `$SYNTHIOS_CONFIG_PATH` (default: `~/.synthios/synthios.json`):

```
$SYNTHIOS_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.synthios/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `SYNTHIOS_GATEWAY_TOKEN`).
...
Synthios enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `synthios doctor --generate-gateway-token`.
...
- `default`: uses `All your chats, one Synthios.` every time.
- `random`: rotating funny/seasonal taglines (default behavior).
- If you want no banner at all, set env `SYNTHIOS_HIDE_BANNER=1`.
...
**Recommended:** run `synthios configure --section web` and choose a provider.
...
- Daemons read env vars from `~/.synthios/.env` (or the service environment).
...
### Can the Synthios browser run headless
...
- Gateway is running: `synthios gateway status`
- Gateway health: `synthios status`
- Channel health: `synthios channels status`
...
### Can two Synthios instances talk to each other local VPS
...
`synthios agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
synthios agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Restore from backup (git or a copied `~/.synthios/synthios.json`).
- If you have no backup, re-run `synthios doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `synthios config set` for small changes.
- Use `synthios configure` for interactive edits.
...
```bash
synthios gateway --tailscale serve
```
...
### How does Synthios load environment variables

Synthios reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.synthios/.env` (aka `$SYNTHIOS_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.synthios/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`SYNTHIOS_LOAD_SHELL_ENV=1`, `SYNTHIOS_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`synthios models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means Synthios won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.synthios/.env`:
...
### Is there a way to make a team of Synthios instances one CEO and many agents
...
### How do I completely reset Synthios but keep it installed
...
```bash
synthios reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `SYNTHIOS_PROFILE`), reset each state dir (defaults are `~/.synthios-<profile>`).
- Dev reset: `synthios gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), Synthios skips the heartbeat run to save API calls.
...
No. Synthios runs on **your own account**, so if you're in the group, Synthios can see it.
...
```bash
synthios directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why does Synthios not reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.synthios/agents/<agentId>/sessions/`.
...
- Use `synthios doctor` to spot stray workspaces and profile mismatches.
...
Synthios's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Synthios currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
4. Run `synthios onboard` and choose `Ollama`
...
- for manual switching, use `synthios models list` and `synthios models set ollama/<model>`
...
- `synthios models set ...` (updates just model config)
- `synthios configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.synthios/synthios.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `synthios doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do Synthios, Flawd, and Krill use for models

- These deployments can differ and may change over time; there is no fixed provider recommendation.
- Check the current runtime setting on each gateway with `synthios models status`.
...
Yes. Synthios ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.synthios/agents/<agentId>/agent/auth-profiles.json
```

Fix options:

- Run `synthios agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so Synthios can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.synthios/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.synthios/agent/*` (migrated by `synthios doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.synthios/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `synthios models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `synthios models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `synthios models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.synthios/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:

    ```bash
    synthios models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Synthios will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: Synthios now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
Synthios uses provider-prefixed IDs like:
...
Synthios may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `synthios models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
synthios models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
synthios models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
synthios models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
synthios models auth order set --provider anthropic --agent main anthropic:default
```

### OAuth vs API key - what is the difference

Synthios supports both:
...
```
--port > SYNTHIOS_GATEWAY_PORT > gateway.port > default 18789
```

### Why does synthios gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `synthios gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does synthios gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `SYNTHIOS_STATE_DIR` mismatch).
...
Synthios enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `synthios gateway --port <port>`.

### How do I run Synthios in remote mode client connects to a Gateway elsewhere
...
- `synthios gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- Fastest: `synthios dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `synthios doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `SYNTHIOS_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- If mismatch persists after the one retry, rotate/re-approve the paired device token:
  - `synthios devices list`
  - `synthios devices rotate --device <id> --role operator`
- Still stuck? Run `synthios status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `SYNTHIOS_CONFIG_PATH` (per-instance config)
- `SYNTHIOS_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `synthios --profile <name> …` per instance (auto-creates `~/.synthios-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `synthios --profile <name> gateway install`.

Profiles also suffix service names (`ai.synthios.<profile>`; legacy `com.synthios.*`, `synthios-gateway-<profile>.service`, `Synthios Gateway (<profile>)`).
...
```
synthios tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/synthios/synthios-YYYY-MM-DD.log
```
...
- macOS: `$SYNTHIOS_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.synthios/logs/...`; profiles use `~/.synthios-<profile>/logs/...`)
- Linux: `journalctl --user -u synthios-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "Synthios Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `synthios gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart Synthios
...
1. Is the Gateway running? `synthios gateway status`
2. Is the Gateway healthy? `synthios status`
3. Does the UI have the right token? `synthios dashboard`
...
```bash
synthios channels status
synthios channels logs --channel telegram
```

Then match the error:

- `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Synthios already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
...
### ELI5 synthios gateway restart vs synthios gateway

- `synthios gateway restart`: restarts the **background service** (launchd/systemd).
- `synthios gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `synthios gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Synthios assistant setup](/start/synthios) and [Agent send](/tools/agent-send).

CLI sending:

```bash
synthios message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose Synthios to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `synthios pairing approve --channel <channel> [--account <id>] <code>`
  - Pending requests are capped at **3 per channel**; check `synthios pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `synthios doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Synthios only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
synthios pairing approve whatsapp <code>
```

List pending requests:

```bash
synthios pairing list whatsapp
```
...
Synthios blocks **cross-provider** messaging by default. If a tool call is bound
