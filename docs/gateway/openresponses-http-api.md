...
Synthios’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`synthios agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "synthios:<agentId>"`, `model: "agent:<agentId>"`, or `x-synthios-agent-id`
- use `x-synthios-session-key` for explicit session routing
