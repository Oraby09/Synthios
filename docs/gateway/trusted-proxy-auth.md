...
read_when:
  - Running Synthios behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Synthios
...
- You run Synthios behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Synthios checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Synthios extracts the user identity from the configured header
...
- Synthios can stay on loopback HTTP behind the proxy.
...
If Synthios itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://synthios.example.com
    to: http://synthios-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
synthios.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy synthios:18789 {
...
location / {
    ...
    proxy_pass http://synthios:18789;
...
`synthios security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Synthios config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `synthios security audit` and review findings
