---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Synthios agents"
read_when:
...
OpenShell is a managed sandbox backend for Synthios. Instead of running Docker
containers locally, Synthios delegates sandbox lifecycle to the `openshell` CLI,
...
- Synthios Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Synthios creates an OpenShell
...
- You edit files locally outside Synthios and want those changes visible in the
...
- When the sandbox is first created, Synthios seeds the remote workspace from
...
- Synthios does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Synthios after the initial seed,
the remote sandbox does **not** see those changes. Use
`synthios sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"synthios"`  | Sandbox source for first-time create                  |
...
1. Synthios calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Synthios calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `synthios sandbox` commands
