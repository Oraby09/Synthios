...
`synthios doctor` is the repair + migration tool for Synthios. It fixes stale
...
```bash
synthios doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
synthios doctor --repair --force
```
...
- Extra workspace dir detection (`~/synthios`).
...
you to run `synthios doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.synthios/synthios.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.synthios/sessions/` to `~/.synthios/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.synthios/agent/` to `~/.synthios/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.synthios/credentials/*.json` (except `oauth.json`)
  - to `~/.synthios/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `synthios doctor`.

### 3b) Legacy cron store migrations

Doctor also checks the cron job store (`~/.synthios/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.synthios` folders exist across
  home directories or when `SYNTHIOS_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.synthios/synthios.json` is
...
offers to remove them and install the Synthios service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Synthios gateway services are considered first-class and are not
flagged as "extra."
...
- `synthios doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `synthios doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `synthios doctor` prompts before rewriting supervisor config.
- `synthios doctor --yes` accepts the default repair prompts.
- `synthios doctor --repair` applies recommended fixes without prompts.
- `synthios doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `synthios gateway install --force`.
