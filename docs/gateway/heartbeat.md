...
- During heartbeat runs, Synthios treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `synthios sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Synthios skips the heartbeat run entirely (no model call).
...
```bash
synthios system event --text "Check for urgent follow-ups" --mode now
```
