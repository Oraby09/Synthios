...
read_when:
  - Setting up Synthios for the first time
...
Synthios reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.synthios/synthios.json`.

If the file is missing, Synthios uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `synthios onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    synthios onboard       # full onboarding flow
    synthios configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    synthios config get agents.defaults.workspace
    synthios config set agents.defaults.heartbeat.every "2h"
    synthios config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.synthios/synthios.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Synthios only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`synthios doctor`, `synthios logs`, `synthios health`, `synthios status`)
- Run `synthios doctor` to see exact issues
- Run `synthios doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@synthios", "synthios"],
            },
          },
        ],
      },
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `synthios.json`.
    ...
    ```bash
    synthios config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `SYNTHIOS_APNS_RELAY_BASE_URL` and `SYNTHIOS_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `SYNTHIOS_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.synthios/synthios.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `synthios config set` for single keys.
    </Warning>
    ...
    ```bash
    synthios gateway call config.get --params '{}'  # capture payload.hash
    synthios gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.synthios/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    synthios gateway call config.patch --params '{
...
Synthios reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.synthios/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Synthios runs your login shell and imports only the missing keys:
...
Env var equivalent: `SYNTHIOS_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${SYNTHIOS_GATEWAY_TOKEN}" } },
