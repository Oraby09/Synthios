...
Synthios supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `synthios gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.synthios/.env` so the daemon can read it:

```bash
cat >> ~/.synthios/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `synthios onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.synthios/.env`, systemd/launchd).
...
```bash
synthios models auth paste-token --provider anthropic
synthios models auth paste-token --provider openrouter
```
...
- Priority order:
  - `SYNTHIOS_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Synthios retries with the next key only for rate-limit errors (for example
...
Run `synthios models status` to confirm which profile is expiring. If the profile
