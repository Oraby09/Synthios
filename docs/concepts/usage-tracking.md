...
- `/usage cost` in chats: local cost summary aggregated from Synthios session logs.
- CLI: `synthios status --usage` prints a full per-provider breakdown.
- CLI: `synthios channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
