---
title: "Memory"
summary: "How Synthios memory works (workspace files + automatic memory flush)"
read_when:
...
Synthios memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, Synthios only loads `MEMORY.md`.
  - Lowercase `memory.md` is only used as a fallback when `MEMORY.md` is absent.
  - **Only load in the main, private session** (never in group contexts).

These files live under the workspace (`agents.defaults.workspace`, default
`~/.synthios/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

Synthios exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, Synthios triggers a **silent,
...
Synthios can build a small vector index over `MEMORY.md` and `memory/*.md` so
