...
read_when:
  - You want to understand how Synthios assembles model context
...
A **context engine** controls how Synthios builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Synthios ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.synthios/synthios.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Synthios plugin. Install
first, then select the engine in the slot:

```bash
# Install from npm
synthios plugins install @martian-engineering/lossless-claw

# Or install from a local path (for development)
synthios plugins install -l ./my-context-engine
```
...
Every time Synthios runs a model prompt, the context engine participates at
...
Synthios currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Synthios
...
The built-in `legacy` engine preserves Synthios's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Synthios uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Synthios disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Synthios automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `synthios/plugin-sdk/core` to use
  Synthios's built-in compaction behavior.
...
Synthios resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Synthios's built-in summarization. Plugin engines can implement
...
- Use `synthios doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Synthios
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `synthios plugins install -l ./my-engine` to link a
