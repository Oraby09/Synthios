...
read_when:
  ...
  - Wiring external systems into Synthios
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Synthios config directory (typically `~/.synthios/hooks/transforms`).
...
- `synthios webhooks gmail setup` writes `hooks.gmail` config for `synthios webhooks gmail run`.
