...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in Synthios.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Synthios. See [Webhook Hooks](/automation/webhook) or use `synthios webhooks` for Gmail helper commands.
...
- Extend Synthios's behavior without modifying core code

## Getting Started

### Bundled Hooks

Synthios ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.synthios/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.synthios/logs/commands.log`
...
During onboarding (`synthios onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.synthios/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<synthios>/dist/hooks/bundled/` (shipped with Synthios)
...
Hook packs are standard npm packages that export one or more hooks via `synthios.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.synthios/hooks/<id>`.
Each `synthios.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `synthios hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.synthios.ai/automation/hooks#my-hook
metadata:
  { "synthios": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.synthios` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Synthios persists them.
...
- **Managed hooks** (`~/.synthios/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.synthios/hooks/my-hook
cd ~/.synthios/hooks/my-hook
```
...
metadata: { "synthios": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
synthios hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.synthios/workspace`)
...
```yaml
metadata: { "synthios": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "synthios": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.synthios/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.synthios/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.synthios/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/synthios/synthios/tree/main/src/hooks/bundled)
