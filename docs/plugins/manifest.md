...
read_when:
  - You are building an Synthios plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (synthios.plugin.json)

This page is for the **native Synthios plugin manifest** only.
...
Synthios auto-detects those bundle layouts too, but they are not validated
against the `synthios.plugin.json` schema described here.

For compatible bundles, Synthios currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Synthios runtime expectations.

Every native Synthios plugin **must** ship a `synthios.plugin.json` file in the
**plugin root**. Synthios uses this manifest to validate configuration
...
- `providerAuthEnvVars` (object): auth env vars keyed by provider id. Use this
  when Synthios should resolve provider credentials from env without loading
  plugin runtime first.
- `providerAuthChoices` (array): cheap onboarding/auth-choice metadata keyed by
  provider + auth method. Use this when Synthios should show a provider in
...
- The manifest is **required for native Synthios plugins**, including local filesystem loads.
