...
summary: "Step-by-step guide for creating Synthios channel and provider extensions"
read_when:
  - You want to create a new Synthios plugin or extension
  - You need to understand the plugin SDK import patterns
  - You are adding a new channel or provider to Synthios
---

# Building Extensions

This guide walks through creating an Synthios extension from scratch. Extensions
can add channels, model providers, tools, or other capabilities.

## Prerequisites

- Synthios repository cloned and dependencies installed (`pnpm install`)
...
├── package.json          # npm metadata + synthios config
...
```json
{
  "name": "@synthios/my-channel",
  "version": "2026.1.1",
  "description": "Synthios My Channel plugin",
  "type": "module",
  "dependencies": {},
  "synthios": {
    "extensions": ["./index.ts"],
...
The `synthios` field tells the plugin system what your extension provides.
...
export default defineChannelPluginEntry({
  id: "my-channel",
  name: "My Channel",
  description: "Connects Synthios to My Channel",
  plugin: {
...
import { createChannelPairingController } from "synthios/plugin-sdk/channel-pairing";
import { createPluginRuntimeStore } from "synthios/plugin-sdk/runtime-store";
import { createOptionalChannelSetupSurface } from "synthios/plugin-sdk/channel-setup";
import { resolveChannelGroupRequireMention } from "synthios/plugin-sdk/channel-policy";

// Wrong: monolithic root (lint will reject this)
import { ... } from "synthios/plugin-sdk";
```
...
Create `synthios.plugin.json` in your extension root:
...
Synthios runs contract tests against all registered plugins. After adding your
extension, run:
...
```typescript
import { createTestRuntime } from "synthios/plugin-sdk/testing";
```
...
1. **No monolithic root imports** — `synthios/plugin-sdk` root is rejected
...
- [ ] `package.json` has correct `synthios` metadata
...
- [ ] `synthios.plugin.json` manifest is present and valid
