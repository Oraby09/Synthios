...
read_when:
  - Building or debugging native Synthios plugins
...
This page covers the internal architecture of the Synthios plugin system. For
user-facing setup, discovery, and configuration, see [Plugins](/tools/plugin).

## Public capability model

Capabilities are the public **native plugin** model inside Synthios. Every
native Synthios plugin registers against one or more capability types:
...
Synthios classifies every loaded plugin into a shape based on its actual
...
Use `synthios plugins inspect <id>` to see a plugin's shape and capability
...
When you run `synthios doctor` or `synthios plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `synthios status --all` and `synthios plugins doctor`.

## Architecture overview

Synthios's plugin system has four layers:

1. **Manifest + discovery**
   Synthios finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `synthios.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Synthios plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Synthios reads the registry to expose tools, channels, provider
...
That split lets Synthios validate config, explain missing/disabled plugins, and
...
normal chat actions. Synthios keeps one shared `message` tool in core, and
...
Synthios treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Synthios-facing
  surfaces
...
So if Synthios adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Synthios has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { SynthiosPluginDefinition } from "synthios/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "synthios/plugin-sdk";

const plugin: SynthiosPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Synthios already treats image/audio/video understanding as one shared
...
If Synthios adds a new domain later, such as video generation, use the same
...
`SynthiosPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Synthios can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Synthios knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `SynthiosPluginApi` or
  `api.runtime`
...
Native Synthios plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Synthios process

Compatible bundles are safer by default because Synthios currently treats them
...
Synthios exports capabilities, not implementation convenience.
...
At startup, Synthios does roughly this:
...
The manifest is the control-plane source of truth. Synthios uses it to:
...
Synthios keeps short in-process caches for:
...
- Set `SYNTHIOS_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `SYNTHIOS_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `SYNTHIOS_PLUGIN_DISCOVERY_CACHE_MS` and
  `SYNTHIOS_PLUGIN_MANIFEST_CACHE_MS`.
...
Synthios still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Synthios calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Synthios tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Synthios's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Synthios's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Synthios adds those
...
- If Synthios adds a new capability such as video generation later, define the
...
- Synthios only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Synthios plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `synthios/plugin-sdk` import when
authoring plugins:

- `synthios/plugin-sdk/plugin-entry` for plugin registration primitives.
- `synthios/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `synthios/plugin-sdk/channel-setup`,
  `synthios/plugin-sdk/channel-pairing`,
  `synthios/plugin-sdk/channel-reply-pipeline`,
  `synthios/plugin-sdk/secret-input`, and
  `synthios/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  wiring.
- Domain subpaths such as `synthios/plugin-sdk/channel-config-helpers`,
  `synthios/plugin-sdk/channel-config-schema`,
  `synthios/plugin-sdk/channel-policy`,
  `synthios/plugin-sdk/channel-runtime`,
  `synthios/plugin-sdk/config-runtime`,
  `synthios/plugin-sdk/agent-runtime`,
  `synthios/plugin-sdk/lazy-runtime`,
  `synthios/plugin-sdk/reply-history`,
  `synthios/plugin-sdk/routing`,
  `synthios/plugin-sdk/runtime-store`, and
  `synthios/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- Narrow channel-core subpaths such as `synthios/plugin-sdk/discord-core`,
  `synthios/plugin-sdk/telegram-core`, and `synthios/plugin-sdk/whatsapp-core`
...
- Bundled extension internals remain private. External plugins should use only
  `synthios/plugin-sdk/*` subpaths. Synthios core/test code may use the repo
...
- `synthios/plugin-sdk/telegram` for Telegram channel plugin types and shared channel-facing helpers. Built-in Telegram implementation internals stay private to the bundled extension.
- `synthios/plugin-sdk/discord` for Discord channel plugin types and shared channel-facing helpers. Built-in Discord implementation internals stay private to the bundled extension.
- `synthios/plugin-sdk/slack` for Slack channel plugin types and shared channel-facing helpers. Built-in Slack implementation internals stay private to the bundled extension.
- `synthios/plugin-sdk/imessage` for iMessage channel plugin types and shared channel-facing helpers. Built-in iMessage implementation internals stay private to the bundled extension.
- `synthios/plugin-sdk/whatsapp` for WhatsApp channel plugin types and shared channel-facing helpers. Built-in WhatsApp implementation internals stay private to the bundled extension.
- `synthios/plugin-sdk/bluebubbles` remains public because it carries a small
  focused helper surface that is shared intentionally.

Compatibility note:

- Avoid the root `synthios/plugin-sdk` barrel for new code.
...
`synthios/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Synthios writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Synthios's
...
- if both `catalog` and `discovery` are registered, Synthios uses `catalog`
...
- Read-only command paths such as `synthios status`, `synthios status --all`,
  `synthios channels status`, `synthios channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `synthios.extensions`:
...
Security guardrail: every `synthios.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `synthios plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `synthios.setupEntry` can point at a lightweight setup-only module.
When Synthios needs setup surfaces for a disabled channel plugin, or
...
Optional: `synthios.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Synthios load the
...
Channel plugins can advertise setup/discovery metadata via `synthios.channel` and
install hints via `synthios.install`. This keeps the core catalog data-free.
...
Synthios can also merge **external channel catalogs** (for example, an MPM
...
Or point `SYNTHIOS_PLUGIN_CATALOG_PATHS` (or `SYNTHIOS_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "synthios": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "synthios/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `SynthiosPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Synthios stays opinionated without becoming hardcoded to one
