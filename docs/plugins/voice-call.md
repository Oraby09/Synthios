...
read_when:
  - You want to place an outbound voice call from Synthios
...
Voice calls for Synthios via a plugin. Supports outbound notifications and
...
- Use `synthios voicecall ...` or the `voice_call` tool
...
```bash
synthios plugins install ./extensions/voice-call
...
synthios voicecall start --to "+15555550123"   # alias for call
...
synthios voicecall latency                     # summarize turn latency from logs
