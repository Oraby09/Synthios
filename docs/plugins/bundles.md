---
summary: "Unified bundle format guide for Codex, Claude, and Cursor bundles in Synthios"
read_when:
  - You want to install or debug a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Synthios maps bundle content into native features
...
Synthios supports one shared class of external plugin package: **bundle
plugins**.
...
Synthios shows all of them as `Format: bundle` in `synthios plugins list`.
Verbose output and `synthios plugins inspect <id>` also show the subtype
...
A bundle is a **content/metadata pack**, not a native in-process Synthios
plugin.

Today, Synthios does **not** execute bundle runtime code in-process. Instead,
it detects known bundle files, reads the metadata, and maps supported bundle
content into native Synthios surfaces such as skills, hook packs, MCP config,
and embedded Pi settings.

That is the main trust boundary:

- native Synthios plugin: runtime module executes in-process
- bundle: metadata/content pack, with selective feature mapping

## Shared bundle model

Codex, Claude, and Cursor bundles are similar enough that Synthios treats them
...
Common Synthios behavior:

- detect the bundle subtype
- normalize it into one internal bundle record
- map supported parts into native Synthios features
...
first. The more useful question is: which bundle surfaces does Synthios map
today?

## Detection order

Synthios prefers native Synthios plugin/package layouts before bundle handling.

Practical effect:

- `synthios.plugin.json` wins over bundle detection
- package installs with valid `package.json` + `synthios.extensions` use the
  native install path
- if a directory contains both native and bundle metadata, Synthios treats it
  as native first
...
Synthios normalizes bundle metadata into one internal bundle record, then maps
...
- bundle skill roots load as normal Synthios skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Synthios skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Synthios hook-pack
...
- Synthios merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Synthios also exposes supported bundle MCP tools during embedded Pi agent
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Synthios sanitizes shell override keys before applying them
...
diagnostics/info output, but Synthios does not run them yet:
...
`synthios plugins inspect <id>` shows bundle capabilities from the normalized
bundle record.
...
```text
bundle capability detected but not wired into Synthios yet: agents
```
...
- Codex `hooks` is considered supported only for Synthios hook-pack layouts

## Format differences

The formats are close, but not byte-for-byte identical. These are the practical
differences that matter in Synthios.
...
Codex bundles fit Synthios best when they use skill roots and Synthios-style
...
Default Claude layout markers Synthios recognizes:
...
Claude bundle manifests can declare custom component paths. Synthios treats
...
- Synthios does not load arbitrary bundle runtime modules in-process
...
```bash
synthios plugins install ./my-codex-bundle
synthios plugins install ./my-claude-bundle
synthios plugins install ./my-cursor-bundle
...
synthios plugins inspect my-bundle
```

If the directory is a native Synthios plugin/package, the native install path
still wins.

For Claude marketplace names, Synthios reads the local Claude known-marketplace
...
Check `synthios plugins inspect <id>`.

If the capability is listed but Synthios says it is not wired yet, that is a
...
Synthios does not treat bundle settings as raw Synthios config patches.

### Claude hooks do not execute

`hooks/hooks.json` is only detected today.

If you need runnable bundle hooks today, use the normal Synthios hook-pack
layout through a supported Codex hook root or ship a native Synthios plugin.
