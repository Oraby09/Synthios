...
read_when:
  - You want Synthios to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Synthios to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`synthios onboard`) and `synthios channels add` list optional channel plugins.
...
```bash
synthios plugins install @synthios/nostr
```

Use a local checkout (dev workflows):

```bash
synthios plugins install --link <path-to-synthios>/extensions/nostr
```
...
synthios channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "synthios",
        displayName: "Synthios",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "synthios@example.com",
        lud16: "synthios@example.com",
      },
    },
  },
}
