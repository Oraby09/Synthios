...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Synthios to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Synthios releases, so no separate plugin install
is required.
...
```bash
synthios plugins install @synthios/feishu
```
...
If you just installed Synthios, run onboarding:
...
2. Configuring app credentials in Synthios
...
1. You already ran `synthios channels add` for Feishu
2. The gateway is running (`synthios gateway status`)
...
```bash
synthios pairing approve feishu <CODE>
```
...
2. Run `synthios logs --follow` and look for `chat_id`
...
2. Run `synthios logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `synthios gateway status`  | Show gateway status           |
| `synthios gateway install` | Install/start gateway service |
| `synthios gateway stop`    | Stop gateway service          |
| `synthios gateway restart` | Restart gateway service       |
| `synthios logs --follow`   | Tail gateway logs             |
...
4. Check logs: `synthios logs --follow`
...
5. Ensure the gateway is running: `synthios gateway status`
6. Check logs: `synthios logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.synthios/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.synthios/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
