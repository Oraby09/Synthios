...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Synthios HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `synthios pairing approve slack <code>`.
...
- The interactive callback values are Synthios-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Synthios falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Synthios is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Synthios",
    "description": "Slack connector for Synthios"
  },
  "features": {
    "bot_user": {
      "display_name": "Synthios",
      "always_online": false
    },
    "app_home": {
...
Synthios supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Synthios falls back to normal delivery for remaining payloads.
