---
summary: "Synology Chat webhook setup and Synthios config"
read_when:
  - Setting up Synology Chat with Synthios
...
```bash
synthios plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `synthios onboard` now shows Synology Chat in the same channel setup list as `synthios channels add`.
   - Non-interactive setup: `synthios channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Synthios gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Synthios.
   - Guided: `synthios onboard`
   - Direct: `synthios channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `synthios pairing list synology-chat`
  - `synthios pairing approve synology-chat <CODE>`
...
```bash
synthios message send --channel synology-chat --target 123456 --text "Hello from Synthios"
synthios message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
