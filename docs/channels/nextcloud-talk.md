...
```bash
synthios plugins install @synthios/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
synthios plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Synthios" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `synthios pairing list nextcloud-talk`
  - `synthios pairing approve nextcloud-talk <CODE>`
