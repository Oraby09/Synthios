...
read_when:
  - Setting up Zalo Personal for Synthios
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Synthios.
...
- Install via CLI: `synthios plugins install @synthios/zalouser`
- Or from a source checkout: `synthios plugins install ./extensions/zalouser`
...
- `synthios pairing list zalouser`
- `synthios pairing approve zalouser <code>`
...
- On startup, Synthios resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Synthios stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Synthios state. Example:
...
- Synthios sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Synthios sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `synthios channels logout --channel zalouser && synthios channels login --channel zalouser`
...
- The channel now runs fully in Synthios without external CLI binaries.
