...
read_when:
  - Setting up Twitch chat integration for Synthios
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Synthios connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
synthios plugins install @synthios/twitch
```

Local checkout (when running from a git repo):

```bash
synthios plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SYNTHIOS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "synthios", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SYNTHIOS_TWITCH_ACCESS_TOKEN env var)
...
```bash
SYNTHIOS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
