...
Tlon is a decentralized messenger built on Urbit. Synthios connects to your Urbit ship and can
...
```bash
synthios plugins install @synthios/tlon
```

Local checkout (when running from a git repo):

```bash
synthios plugins install ./extensions/tlon
```
...
By default, Synthios blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `synthios message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Synthios replies in-thread.
