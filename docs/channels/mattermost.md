---
summary: "Mattermost bot setup and Synthios config"
read_when:
...
```bash
synthios plugins install @synthios/mattermost
```

Local checkout (when running from a git repo):

```bash
synthios plugins install ./extensions/mattermost
```
...
4. Configure Synthios and start the gateway.
...
Native slash commands are opt-in. When enabled, Synthios registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Synthios derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Synthios.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Synthios.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Synthios, not `404`.
...
- Approve via:
  - `synthios pairing list mattermost`
  - `synthios pairing approve mattermost <CODE>`
...
Use these target formats with `synthios message send` or cron/webhooks:
...
Synthios resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Synthios sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Synthios sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Synthios derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Synthios run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="synthios-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"synthios-mattermost-interactions",
...
`synthios message send` and cron/webhook deliveries.
