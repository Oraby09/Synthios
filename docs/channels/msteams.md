...
```bash
synthios plugins install @synthios/msteams
```

Local checkout (when running from a git repo):

```bash
synthios plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
Synthios will offer the local install path automatically.
...
3. Configure Synthios with those credentials.
...
- Talk to Synthios via Teams DMs, group chats, or channels.
...
- On startup, Synthios resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.synthios/synthios.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Synthios, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `synthios-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `synthios plugins install @synthios/msteams`
   - From a local checkout: `synthios plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "Synthios in Teams", full: "Synthios in Teams" },
...
Synthios handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Synthios only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SynthiosShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Synthios sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `synthios message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.synthios/msteams-polls.json`.
...
```bash
synthios message send --channel msteams \
...
synthios message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
synthios message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
synthios message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
synthios message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
