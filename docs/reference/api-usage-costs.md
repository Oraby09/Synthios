...
Synthios features that can generate provider usage or paid API calls.
...
- `synthios status --usage` and `synthios channels list` show provider **usage windows**
...
Synthios can pick up credentials from:
...
- `synthios status --usage`
- `synthios models status --json`
...
`synthios models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
