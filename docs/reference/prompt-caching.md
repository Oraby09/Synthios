...
- With Anthropic API-key auth profiles, Synthios seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Synthios injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Synthios exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.synthios/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$SYNTHIOS_STATE_DIR/logs/cache-trace.jsonl`
...
- `SYNTHIOS_CACHE_TRACE=1` enables cache tracing.
- `SYNTHIOS_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `SYNTHIOS_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `SYNTHIOS_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `SYNTHIOS_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
