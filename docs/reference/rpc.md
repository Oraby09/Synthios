...
Synthios integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Synthios owns lifecycle when `channels.signal.autoStart=true`.
...
- Synthios spawns `imsg rpc` as a child process (legacy iMessage integration).
