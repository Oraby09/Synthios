---
summary: "How Synthios builds prompt context and reports token usage + costs"
read_when:
...
Synthios tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Synthios assembles its own system prompt on every run. It includes:
...
For images, Synthios downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Synthios session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `synthios status --usage` and `synthios channels list` show
...
`cacheWrite`. If pricing is missing, Synthios shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Synthios can
...
Anthropic's 1M context window is currently beta-gated. Synthios can inject the
...
Synthios skips the `context-1m-*` beta header because Anthropic currently
