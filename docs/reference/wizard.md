...
This is the full reference for `synthios onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.synthios/synthios.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `synthios doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.synthios/credentials/oauth.json`; auth profiles live in `~/.synthios/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.synthios/credentials/oauth.json` (or `$SYNTHIOS_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.synthios/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `synthios pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `synthios configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `synthios health`.
    - Tip: `synthios status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export SYNTHIOS_GATEWAY_TOKEN="your-token"
synthios onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env SYNTHIOS_GATEWAY_TOKEN
```
...
- Stores it under `~/.synthios/tools/signal-cli/<version>/`.
...
Typical fields in `~/.synthios/synthios.json`:
...
`synthios agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.synthios/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.synthios/agents/<agentId>/sessions/`.
