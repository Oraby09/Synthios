---
summary: "CLI reference for `synthios configure` (interactive configuration prompts)"
read_when:
...
Tip: `synthios config` without a subcommand opens the same wizard. Use
`synthios config get|set|unset` for non-interactive edits.
...
```bash
synthios configure
synthios configure --section model --section channels
```
