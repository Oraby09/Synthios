...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Synthios Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`synthios acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Synthios gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
synthios acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
synthios acp --url wss://gateway-host:18789 --token-file ~/.synthios/gateway.token

# Attach to an existing session key
synthios acp --session agent:main:main

# Attach by label (must already exist)
synthios acp --session-label "support inbox"

# Reset the session key before the first prompt
synthios acp --session agent:main:main --reset-session
```
...
synthios acp client --server-args --url wss://gateway-host:18789 --token-file ~/.synthios/gateway.token

# Override the server command (default: synthios)
synthios acp client --server "node" --server-args synthios.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Synthios Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `synthios acp` over stdio.

Example config (persisted):

```bash
synthios config set gateway.remote.url wss://gateway-host:18789
synthios config set gateway.remote.token <token>
```
...
```bash
synthios acp --session agent:main:main
synthios acp --session agent:design:main
synthios acp --session agent:qa:bug-123
```
...
Synthios bot over ACP, use `acpx` with its built-in `synthios` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx synthios` at `synthios acp`.
3. Target the Synthios session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Synthios ACP session
acpx synthios exec "Summarize the active Synthios session state."

# Persistent named session for follow-up turns
acpx synthios sessions ensure --name codex-bridge
acpx synthios -s codex-bridge --cwd /path/to/repo \
  "Ask my Synthios work agent for recent context relevant to this repo."
```

If you want `acpx synthios` to target a specific Gateway and session key every
time, override the `synthios` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "synthios": {
      "command": "env SYNTHIOS_HIDE_BANNER=1 SYNTHIOS_SUPPRESS_NOTES=1 synthios acp --url ws://127.0.0.1:18789 --token-file ~/.synthios/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Synthios checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env SYNTHIOS_HIDE_BANNER=1 SYNTHIOS_SUPPRESS_NOTES=1 node synthios.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Synthios agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Synthios ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`SYNTHIOS_GATEWAY_TOKEN`, `SYNTHIOS_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`SYNTHIOS_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `SYNTHIOS_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `synthios acp client` sets `SYNTHIOS_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `synthios`).
