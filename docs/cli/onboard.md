---
summary: "CLI reference for `synthios onboard` (interactive onboarding)"
read_when:
...
```bash
synthios onboard
synthios onboard --flow quickstart
synthios onboard --flow manual
synthios onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`SYNTHIOS_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `synthios gateway run`.
...
- Fastest first chat: `synthios dashboard` (Control UI, no channel setup).
