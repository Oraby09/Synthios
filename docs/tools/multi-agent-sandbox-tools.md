...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `synthios sandbox explain`.
...
Legacy `agent.*` configs are migrated by `synthios doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=synthios-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SYNTHIOS_STATE_DIR:-$HOME/.synthios}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
