...
read_when:
  - Running Synthios Gateway in WSL2 while Chrome lives on Windows
...
- Synthios Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/synthios/synthios/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Synthios and Chrome are on the same machine
...
- Synthios points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Synthios is not the problem yet.
...
Fix that before touching Synthios config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Synthios at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Synthios to succeed
...
```bash
synthios browser open https://example.com --browser-profile remote
synthios browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `synthios browser tabs` returns the target
...
3. Synthios config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Synthios config or Control UI auth
