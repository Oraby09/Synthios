---
summary: "Agent tool surface for Synthios (browser, canvas, nodes, message, cron) replacing legacy `synthios-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `synthios-*` skills
title: "Tools"
---

# Tools (Synthios)

Synthios exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `synthios-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `synthios.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Synthios logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:synthios`: all built-in Synthios tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`synthios node run`).
...
Synthios tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `synthios configure --section web`).
...
Control the dedicated Synthios-managed browser.
...
- Omit `profile` for the safe default: isolated Synthios-managed browser (`synthios`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `synthios`).
...
- Quick smoke: `synthios nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- Available when `agents.defaults.imageGenerationModel` is configured, or when Synthios can infer a compatible image-generation default from your enabled providers plus available auth.
...
- When editing and `resolution` is omitted, Synthios infers a draft/final resolution from the input image size.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `synthios gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Synthios uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Synthios clamps `tools.sessions.visibility` to `tree`.
