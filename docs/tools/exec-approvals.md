...
- **gateway host** → `synthios` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Synthios also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.synthios/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.synthios/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `synthios approvals allowlist ...`).
- `synthios security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `synthios doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.synthios/exec-approvals.json` directly.

CLI: `synthios approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Synthios does not treat
...
topics, Synthios preserves the topic for the approval prompt and the post-approval follow-up.
