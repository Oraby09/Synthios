...
Synthios supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Synthios also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `synthios configure --section web`. It stores the key in
`~/.synthios/synthios.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.synthios/.env` (or your service environment). See [Env vars](/help/faq#how-does-synthios-load-environment-variables).
