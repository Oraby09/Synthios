...
Synthios can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `synthios configure --section web` enables the bundled Firecrawl plugin automatically.
...
Synthios always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
