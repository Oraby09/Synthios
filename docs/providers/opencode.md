---
summary: "Use OpenCode Zen and Go catalogs with Synthios"
read_when:
...
OpenCode exposes two hosted catalogs in Synthios:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Synthios keeps the runtime provider ids
...
synthios onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
synthios onboard --auth-choice opencode-go
synthios onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
