---
summary: "Run Synthios with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Synthios against a local SGLang server
...
Synthios can connect to SGLang using the `openai-completions` API.

Synthios can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Synthios will query:
