---
summary: "Use Venice AI privacy-focused models in Synthios"
read_when:
  - You want privacy-focused inference in Synthios
...
```bash
synthios onboard --auth-choice venice-api-key
```
...
```bash
synthios agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Synthios shows all available Venice models. Pick based on your needs:
...
```bash
synthios models set venice/kimi-k2-5
synthios models set venice/claude-opus-4-6
```

List all available models:

```bash
synthios models list | grep venice
```

## Configure via `synthios configure`

1. Run `synthios configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Synthios automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
synthios agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
synthios agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
synthios agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
synthios agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
synthios agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `synthios models list` to see currently available models. Some models may be temporarily offline.
