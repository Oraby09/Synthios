---
summary: "Use MiniMax M2.5 in Synthios"
read_when:
  - You want MiniMax models in Synthios
...
```bash
synthios plugins enable minimax  # skip if already loaded.
synthios gateway restart  # restart if gateway is already running
synthios onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/synthios/synthios/tree/main/extensions/minimax) for details.
...
Configure manually via `synthios.json`:
...
- Use `synthios models list` and `synthios models set minimax/MiniMax-M2.5` to switch.
...
- Running `synthios configure` and selecting **MiniMax M2.5**, or
