...
read_when:
  - You want to use Groq with Synthios
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Synthios connects
...
media-understanding provider, Synthios uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.synthios/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `synthios models list | grep groq`
