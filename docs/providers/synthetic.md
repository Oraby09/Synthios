---
summary: "Use Synthetic's Anthropic-compatible API in Synthios"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Synthios registers it as the
...
```bash
synthios onboard --auth-choice synthetic-api-key
```
...
Note: Synthios's Anthropic client appends `/v1` to the base URL, so use
