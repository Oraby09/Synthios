...
你可以在 `~/.synthios/synthios.json` 中覆盖此设置：
...
所有日志配置都在 `~/.synthios/synthios.json` 的 `logging` 下。
...
- Synthios 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `synthios plugins enable diagnostics-otel` 启用插件。
...
- `synthios.tokens`（计数器，属性：`synthios.token`、`synthios.channel`、`synthios.provider`、`synthios.model`）
- `synthios.cost.usd`（计数器，属性：`synthios.channel`、`synthios.provider`、`synthios.model`）
- `synthios.run.duration_ms`（直方图，属性：`synthios.channel`、`synthios.provider`、`synthios.model`）
- `synthios.context.tokens`（直方图，属性：`synthios.context`、`synthios.channel`、`synthios.provider`、`synthios.model`）

消息流：

- `synthios.webhook.received`（计数器，属性：`synthios.channel`、`synthios.webhook`）
- `synthios.webhook.error`（计数器，属性：`synthios.channel`、`synthios.webhook`）
- `synthios.webhook.duration_ms`（直方图，属性：`synthios.channel`、`synthios.webhook`）
- `synthios.message.queued`（计数器，属性：`synthios.channel`、`synthios.source`）
- `synthios.message.processed`（计数器，属性：`synthios.channel`、`synthios.outcome`）
- `synthios.message.duration_ms`（直方图，属性：`synthios.channel`、`synthios.outcome`）

队列 + 会话：

- `synthios.queue.lane.enqueue`（计数器，属性：`synthios.lane`）
- `synthios.queue.lane.dequeue`（计数器，属性：`synthios.lane`）
- `synthios.queue.depth`（直方图，属性：`synthios.lane` 或 `synthios.channel=heartbeat`）
- `synthios.queue.wait_ms`（直方图，属性：`synthios.lane`）
- `synthios.session.state`（计数器，属性：`synthios.state`、`synthios.reason`）
- `synthios.session.stuck`（计数器，属性：`synthios.state`）
- `synthios.session.stuck_age_ms`（直方图，属性：`synthios.state`）
- `synthios.run.attempt`（计数器，属性：`synthios.attempt`）
...
- `synthios.webhook.error`
  - `synthios.channel`、`synthios.webhook`、`synthios.chatId`、`synthios.error`
- `synthios.message.processed`
  - `synthios.channel`、`synthios.outcome`、`synthios.chatId`、`synthios.messageId`、`synthios.sessionKey`、`synthios.sessionId`、`synthios.reason`
- `synthios.session.stuck`
  - `synthios.state`、`synthios.ageMs`、`synthios.queueDepth`、`synthios.sessionKey`、`synthios.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `synthios doctor`。
