...
- `~/Library/Application Support/Synthios/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `synthios-canvas://<session>/<path>`

示例：

- `synthios-canvas://main/` → `<canvasRoot>/main/index.html`
- `synthios-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `synthios-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
synthios nodes canvas present --node <id>
synthios nodes canvas navigate --node <id> --url "/"
synthios nodes canvas eval --node <id> --js "document.title"
synthios nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__synthios__/a2ui/
```
...
synthios nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
synthios nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "synthios://agent?message=Review%20this%20design";
```
