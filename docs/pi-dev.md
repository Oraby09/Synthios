...
This guide summarizes a sane workflow for working on the pi integration in Synthios.
...
```bash
SYNTHIOS_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm synthios agent --message "Hello" --thinking low`
...
State lives under the Synthios state directory. Default is `~/.synthios`. If `SYNTHIOS_STATE_DIR` is set, use that directory instead.

To reset everything:

- `synthios.json` for config
