...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Synthios
...
Running Synthios via Node with `tsx` fails at startup with:

```
[synthios] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `synthios status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch synthios.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node synthios.mjs status` works on Node 25.
