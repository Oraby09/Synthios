---
summary: "Deploy Synthios on Railway with one-click template"
read_when:
  - Deploying Synthios to Railway
  - You want a one-click cloud deploy with browser-based setup
title: "Railway"
---

Deploy Synthios on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/synthios` — Control UI
...
- `SYNTHIOS_STATE_DIR=/data/.synthios` (recommended)
- `SYNTHIOS_WORKSPACE_DIR=/data/workspace` (recommended)
- `SYNTHIOS_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Synthios state + workspace so you can migrate to another host without losing config or memory.
