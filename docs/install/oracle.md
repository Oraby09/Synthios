---
summary: "Host Synthios on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Synthios on Oracle Cloud
  - Looking for free VPS hosting for Synthios
...
Run a persistent Synthios Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@synthios`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=synthios --reset` to re-authenticate.

**Gateway will not start** -- Run `synthios doctor --non-interactive` and check logs with `journalctl --user -u synthios-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Synthios up to date
