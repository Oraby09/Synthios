---
summary: "Optional Docker-based setup and onboarding for Synthios"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Synthios on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export SYNTHIOS_IMAGE="ghcr.io/synthios/synthios:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/synthios/synthios/pkgs/container/synthios).
...
<Note>
Run `docker compose` from the repo root. If you enabled `SYNTHIOS_EXTRA_MOUNTS`
or `SYNTHIOS_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `SYNTHIOS_IMAGE`               | Use a remote image instead of building locally                   |
| `SYNTHIOS_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `SYNTHIOS_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `SYNTHIOS_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `SYNTHIOS_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `SYNTHIOS_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `SYNTHIOS_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `SYNTHIOS_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `SYNTHIOS_CONFIG_DIR` to `/home/node/.synthios` and
`SYNTHIOS_WORKSPACE_DIR` to `/home/node/.synthios/workspace`, so those paths
...
and rolling file logs under `/tmp/synthios/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/synthios/synthios/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/synthios/synthios/blob/main/scripts/shell-helpers/README.md).

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export SYNTHIOS_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export SYNTHIOS_SANDBOX=1
    export SYNTHIOS_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm synthios-cli gateway probe
    docker compose run -T --rm synthios-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `synthios-cli` uses `network_mode: "service:synthios-gateway"` so CLI
    ...
    `no-new-privileges` on `synthios-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.synthios`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export SYNTHIOS_HOME_VOLUME="synthios_home"`
    2. **Bake system deps**: `export SYNTHIOS_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `SYNTHIOS_HOME_VOLUME` or `SYNTHIOS_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/synthios/synthios/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Synthios runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm synthios-cli config set gateway.mode local
    docker compose run --rm synthios-cli config set gateway.bind lan
    docker compose run --rm synthios-cli devices list --url ws://127.0.0.1:18789
    ```
