---
summary: "Run Synthios Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Synthios running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Synthios Gateway on your own Azure Linux VM
...
# Synthios on Azure Linux VM

This guide sets up an Azure Linux VM, applies Network Security Group (NSG) hardening, configures Azure Bastion (managed Azure SSH entry point), and installs Synthios.
...
- Install Synthios with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-synthios"
    LOCATION="westus2"
    TEMPLATE_URI="https://raw.githubusercontent.com/synthios/synthios/main/infra/azure/templates/azuredeploy.json"
    PARAMS_URI="https://raw.githubusercontent.com/synthios/synthios/main/infra/azure/templates/azuredeploy.parameters.json"
    ```
  </Step>
  ...
  <Step title="SSH into the VM through Azure Bastion">
    ...
    RG="rg-synthios"
    VM_NAME="vm-synthios"
    BASTION_NAME="bas-synthios"
    ADMIN_USERNAME="synthios"
  ...
  <Step title="Install Synthios (in the VM shell)">
    ```bash
    curl -fsSL https://synthios.ai/install.sh -o /tmp/synthios-install.sh
    bash /tmp/synthios-install.sh
    rm -f /tmp/synthios-install.sh
    synthios --version
    ```
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Synthios onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- For more details on Synthios Azure deployment with the GitHub Copilot model provider: [Synthios on Azure with GitHub Copilot](https://github.com/johnsonshi/synthios-azure-github-copilot)
