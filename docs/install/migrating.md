---
summary: "Move (migrate) an Synthios install from one machine to another"
read_when:
  - You are moving Synthios to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Synthios to a New Machine

This guide moves an Synthios gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.synthios/` by default) and your **workspace**, you preserve:

- **Config** -- `synthios.json` and all gateway settings
...
<Tip>
Run `synthios status` on the old machine to confirm your state directory path.
Custom profiles use `~/.synthios-<profile>/` or a path set via `SYNTHIOS_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf synthios-state.tgz .synthios
    ```

    If you use multiple profiles (e.g. `~/.synthios-work`), archive each separately.

  </Step>

  <Step title="Install Synthios on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.synthios/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `SYNTHIOS_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `synthios doctor`.
  </Accordion>

  <Accordion title="Copying only synthios.json">
  ...
  <Accordion title="Remote mode">
    ...
    Migrate the gateway host itself, not your local laptop. See [FAQ](/help/faq#where-does-synthios-store-its-data).
  </Accordion>
...
- [ ] `synthios status` shows the gateway running
