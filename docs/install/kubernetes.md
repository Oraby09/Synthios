---
summary: "Deploy Synthios Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Synthios on a Kubernetes cluster
  - You want to test Synthios in a Kubernetes environment
title: "Kubernetes"
---

# Synthios on Kubernetes

A minimal starting point for running Synthios on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.
...
kubectl port-forward svc/synthios 18789:18789 -n synthios
...
```bash
kubectl get secret synthios-secrets -n synthios -o jsonpath='{.data.SYNTHIOS_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: synthios (configurable via SYNTHIOS_NAMESPACE)
├── Deployment/synthios        # Single pod, init container + gateway
├── Service/synthios           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/synthios-config  # synthios.json + AGENTS.md
└── Secret/synthios-secrets    # Gateway token + API keys
```
...
Edit `synthios.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret synthios-secrets -n synthios \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/synthios -n synthios
```

### Custom namespace

```bash
SYNTHIOS_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/synthios/synthios:latest # or pin to a specific version from https://github.com/synthios/synthios/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # synthios.json + AGENTS.md
