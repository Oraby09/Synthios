...
Synthios ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/synthios`, override with
  `SYNTHIOS_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
synthios update --tag synthios@2026.3.12
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `synthios update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Synthios prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `synthios update` would do without making changes:

```bash
synthios update --dry-run
synthios update --channel beta --dry-run
synthios update --tag 2026.3.14 --dry-run
synthios update --dry-run --json
```
...
When you switch channels with `synthios update`, Synthios also syncs plugin
sources:
