---
summary: "Deploy Synthios on Render with Infrastructure-as-Code"
read_when:
  - Deploying Synthios to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Synthios on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/synthios/synthios)
...
repository configures everything needed to run Synthios:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SYNTHIOS_STATE_DIR
        value: /data/.synthios
      - key: SYNTHIOS_WORKSPACE_DIR
        value: /data/workspace
      - key: SYNTHIOS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/synthios`.
...
If you use the original Synthios repository, Render will not auto-deploy your Synthios. To update it, run a manual Blueprint sync from the dashboard.
...
For Synthios, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Synthios host.
