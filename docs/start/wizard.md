...
CLI onboarding is the **recommended** way to set up Synthios on macOS,
...
`synthios dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`synthios configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.synthios/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `synthios doctor` first.
</Note>
...
Use `synthios agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.synthios/workspace-<agentId>`.
...
- CLI command reference: [`synthios onboard`](/cli/onboard)
