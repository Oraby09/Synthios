...
read_when:
  - You need detailed behavior for synthios onboard
...
This page is the full reference for `synthios onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.synthios/synthios.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `synthios doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `synthios pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `synthios health`.
    - `synthios status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.synthios/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.synthios/agents/<agentId>/agent/auth-profiles.json`
...
`~/.synthios/credentials/oauth.json` (or `$SYNTHIOS_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`synthios onboard`](/cli/onboard)
