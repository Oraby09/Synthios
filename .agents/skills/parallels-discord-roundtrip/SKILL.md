...
```bash
export SYNTHIOS_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.synthios/synthios.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env SYNTHIOS_PARALLELS_DISCORD_TOKEN \
...
- Use the `synthios` wrapper for guest `message send/read`; `node synthios.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/synthios-parallels-smoke.*`
...
- host inbound nonce appears in `synthios message read` output
