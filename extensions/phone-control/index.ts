...
function normalizeDenyList(cfg: SynthiosPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SynthiosPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: SynthiosPluginApi) {
    ...
    const timerService: SynthiosPluginService = {
