...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["synthiosPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["synthios", "pollId"]) ??
    readNestedString(value, ["synthios", "poll", "id"]) ??
    readNestedString(value, ["data", "synthiosPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "synthios", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          synthiosPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "synthios poll vote",
          displayText: "Vote recorded",
          value: { synthiosPollId: pollId, pollId },
        },
      },
    ],
  };
