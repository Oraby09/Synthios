...
`synthios.invoke --each`) without adding new Synthios code per workflow.
...
This extension depends on Synthios internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Synthios extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.synthios/extensions` as a standalone plugin directory.
