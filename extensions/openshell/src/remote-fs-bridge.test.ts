...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("synthios-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("synthios-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("synthios-openshell-remote-agent-");
