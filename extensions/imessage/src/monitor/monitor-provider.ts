...
import { normalizeScpRemoteHost } from "synthios/plugin-sdk/infra-runtime";
import { waitForTransportReady } from "synthios/plugin-sdk/infra-runtime";
import {
...
import { danger, logVerbose, shouldLogVerbose, warn } from "synthios/plugin-sdk/runtime-env";
 ...
 *   exec ssh -T synthios@192.168.64.3 /opt/homebrew/bin/imsg "$@"
...
async function detectRemoteHostFromCliPath(cliPath: string): Promise<string | undefined> {
  try {
    ...
    // Match user@host pattern first (e.g., synthios@192.168.64.3)
