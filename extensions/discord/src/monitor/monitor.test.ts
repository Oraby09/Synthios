...
import type { DiscordAccountConfig } from "synthios/plugin-sdk/config-runtime";
import { buildPluginBindingApprovalCustomId } from "synthios/plugin-sdk/conversation-runtime";
import { buildAgentSessionKey } from "synthios/plugin-sdk/routing";
...
vi.mock("synthios/plugin-sdk/security-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/security-runtime")>();
  return {
    ...actual,
...
vi.mock("synthios/plugin-sdk/conversation-runtime", async (importOriginal) => {
...
vi.mock("synthios/plugin-sdk/infra-runtime", async (importOriginal) => {
...
vi.mock("synthios/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.mock("synthios/plugin-sdk/channel-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/channel-runtime")>();
  return {
    ...actual,
...
vi.mock("synthios/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.mock("synthios/plugin-sdk/plugin-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/plugin-runtime")>();
  return {
    ...actual,
...
describe("agent components", () => {
  const createCfg = (): SynthiosConfig => ({}) as SynthiosConfig;
  ...
  it("sends pairing reply when DM sender is not allowlisted", async () => {
    ...
    expect(pairingText).toContain(`synthios pairing approve discord ${code}`);
...
describe("discord component interactions", () => {
  const createCfg = (): SynthiosConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/synthios-sessions-test.json");
    ...
    resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
      ...
      binding: {
        ...
        pluginName: "Synthios App Server",
