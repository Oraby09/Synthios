...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): SynthiosConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("synthios/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("synthios/plugin-sdk/plugin-runtime")>(
    "synthios/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
