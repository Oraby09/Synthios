...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): SynthiosConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as SynthiosConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): SynthiosConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as SynthiosConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: SynthiosConfig, accountId = "media") {
    await handleTelegramAction(
