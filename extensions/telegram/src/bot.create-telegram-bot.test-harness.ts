...
import { resetInboundDedupe } from "synthios/plugin-sdk/reply-runtime";
import type { MsgContext } from "synthios/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "synthios/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "synthios/plugin-sdk/reply-runtime";
import type { MockFn } from "synthios/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("synthios/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("synthios/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("synthios/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/synthios-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("synthios/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("synthios/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("synthios/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("synthios/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("synthios/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: SynthiosConfig): {
...
vi.doMock("synthios/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("synthios/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("synthios/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: SynthiosConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: SynthiosConfig) => {
