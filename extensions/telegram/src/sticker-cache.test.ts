...
vi.mock("synthios/plugin-sdk/agent-runtime", () => ({
...
vi.mock("synthios/plugin-sdk/media-runtime", () => ({
  AUTO_IMAGE_KEY_PROVIDERS: ["openai"],
  DEFAULT_IMAGE_MODELS: { openai: "gpt-4.1-mini" },
  resolveAutoImageModel: vi.fn(async () => null),
}));

vi.mock("synthios/plugin-sdk/media-understanding-runtime", () => ({
  describeImageFileWithModel: vi.fn(),
}));

const TEST_CACHE_DIR = "/tmp/synthios-test-sticker-cache/telegram";
...
describe("sticker-cache", () => {
  beforeEach(async () => {
    process.env.SYNTHIOS_STATE_DIR = "/tmp/synthios-test-sticker-cache";
    fs.rmSync("/tmp/synthios-test-sticker-cache", { recursive: true, force: true });
