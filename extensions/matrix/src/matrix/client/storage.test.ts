...
import { resolveMatrixAccountStorageRoot } from "synthios/plugin-sdk/matrix";
...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-synthios-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "synthios-matrix-storage-"));
    const stateDir = path.join(homeDir, ".synthios");
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      SYNTHIOS_HOME: path.dirname(stateDir),
      SYNTHIOS_STATE_DIR: stateDir,
      SYNTHIOS_TEST_FAST: "1",
