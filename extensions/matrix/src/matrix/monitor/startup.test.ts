...
const hoisted = vi.hoisted(() => ({
  ...
  summarizeMatrixDeviceHealth: vi.fn(() => ({
    staleSynthiosDevices: [] as Array<{ deviceId: string }>,
  })),
...
describe("runMatrixStartupMaintenance", () => {
  beforeEach(() => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockClear().mockReturnValue({ staleSynthiosDevices: [] });
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockReturnValue({
      staleSynthiosDevices: [{ deviceId: "DEV123" }],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Synthios devices detected for @bot:example.org: DEV123. Run 'synthios matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'synthios matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
