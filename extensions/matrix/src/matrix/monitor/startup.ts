...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleSynthiosDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Synthios devices detected for ${params.auth.userId}: ${deviceHealth.staleSynthiosDevices.map((device) => device.deviceId).join(", ")}. Run 'synthios matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
