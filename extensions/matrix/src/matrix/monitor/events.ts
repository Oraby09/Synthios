...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Synthios runtime is using the same account. " +
    `Check 'synthios matrix verify status --verbose --account ${accountId}' and 'synthios matrix devices list --account ${accountId}'.`
  );
}
