...
vi.mock("synthios/plugin-sdk/routing", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/routing")>();
  return {
    ...actual,
...
export function resetSlackSlashMocks() {
  ...
  mocks.resolveStorePathMock.mockReset().mockReturnValue("/tmp/synthios-sessions.json");
}
