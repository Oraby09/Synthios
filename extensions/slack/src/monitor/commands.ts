import type { SlackSlashCommandConfig } from "synthios/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "synthios");
  const name = normalizedName || "synthios";
  return {
