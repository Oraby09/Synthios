...
// Prefix for Synthios-generated action IDs to scope our handler
const SYNTHIOS_ACTION_PREFIX = "synthios:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${SYNTHIOS_ACTION_PREFIX}`);

  // Handle Synthios modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
