...
import { normalizeSecretInput } from "synthios/plugin-sdk/provider-auth";
...
type WebSearchConfig = NonNullable<SynthiosConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<SynthiosConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SynthiosConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: SynthiosConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: SynthiosConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: SynthiosConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: SynthiosConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: SynthiosConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: SynthiosConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: SynthiosConfig, override?: number): number {
