...
import { createAccountStatusSink } from "synthios/plugin-sdk/channel-lifecycle";
import {
...
function resolveZalouserOutboundChunkMode(cfg: SynthiosConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: SynthiosConfig, accountId?: string) {
