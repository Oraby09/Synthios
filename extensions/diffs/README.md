...
Read-only diff viewer plugin for **Synthios** agents.
...
Set plugin-wide defaults in `~/.synthios/synthios.json`:
...
Before:
Synthios supports plugins.

After:
Synthios supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/synthios-diffs`).
