...
import type { SynthiosPluginApi, SynthiosPluginToolContext } from "./api.js";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<SynthiosPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: SynthiosPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<SynthiosPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as SynthiosPluginApi);
