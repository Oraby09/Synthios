...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("synthios-diffs-tool-"));
  });
...
function createApi(): SynthiosPluginApi {
  return createTestPluginApi({
    ...
    runtime: {} as SynthiosPluginApi["runtime"],
  }) as SynthiosPluginApi;
}
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: SynthiosPluginToolContext | undefined = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
