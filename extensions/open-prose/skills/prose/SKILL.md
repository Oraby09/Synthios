...
metadata: { "synthios": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Synthios `sessions_spawn`
- **File I/O** == Synthios `read`/`write`
- **Remote fetch** == Synthios `web_fetch` (or `exec` with curl when POST is required)
