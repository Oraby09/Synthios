...
const CONFIG_KEY = Symbol.for("synthios:testConfigMock");
...
vi.mock("synthios/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/synthios-oauth",
  };
});

vi.mock("@whiskeysockets/baileys", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("synthios:lastSocket")] =
    created.lastSocket;
  return created.mod;
});
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("synthios:lastSocket")];
  if (typeof getter === "function") {
