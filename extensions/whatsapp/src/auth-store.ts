...
import { info, success } from "synthios/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "synthios/plugin-sdk/state-paths";
import type { WebChannel } from "synthios/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "synthios/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("synthios channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
