...
function mergeWhatsAppConfig(
  cfg: SynthiosConfig,
  patch: Partial<NonNullable<NonNullable<SynthiosConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: SynthiosConfig, dmPolicy: DmPolicy): SynthiosConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: SynthiosConfig, allowFrom?: string[]): SynthiosConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: SynthiosConfig, selfChatMode: boolean): SynthiosConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: SynthiosConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Synthios can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Synthios" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("synthios channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
