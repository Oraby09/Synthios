...
import { resetLogger, setLoggerOverride } from "synthios/plugin-sdk/runtime-env";
...
vi.mock("synthios/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("synthios/plugin-sdk/media-runtime")>();
  return {
    ...actual,
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "synthios-auth-"));
    } else {
      authDir = undefined;
    }
  });
