...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as SynthiosConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as SynthiosConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as SynthiosConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as SynthiosConfig);
