...
function resolveGatewayPort(cfg: SynthiosPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.SYNTHIOS_GATEWAY_PORT?.trim()) ??
...
function resolveAuthLabel(cfg: SynthiosPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: SynthiosPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for Synthios devices",
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (channel === "telegram" && target) {
            try {
              ...
              if (send) {
                await send(
                  target,
                  ["Scan this QR code with the Synthios iOS app:", "", "```", qrAscii, "```"].join(
                    "\n",
                  ),
                  {
          ...
          return {
            text: [
              "Scan this QR code with the Synthios iOS app:",
              "",
              "```",
              qrAscii,
              "```",
