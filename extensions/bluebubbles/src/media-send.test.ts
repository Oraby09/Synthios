...
import type { SynthiosConfig, PluginRuntime } from "synthios/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): SynthiosConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "synthios-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
