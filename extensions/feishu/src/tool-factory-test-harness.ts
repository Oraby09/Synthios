import type { AnyAgentTool, SynthiosPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: SynthiosPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<SynthiosPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
