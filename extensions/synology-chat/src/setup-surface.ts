...
function getChannelConfig(cfg: SynthiosConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: SynthiosConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: SynthiosConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: SynthiosConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Synthios uses to send replies back to Chat.",
      ],
