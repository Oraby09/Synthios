import type { SynthiosConfig } from "synthios/plugin-sdk/discord";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: SynthiosConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): SynthiosConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("synthios/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("synthios/plugin-sdk/acp-runtime")>(
    "synthios/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("synthios/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("synthios/plugin-sdk/reply-runtime")>(
    "synthios/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("synthios/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("synthios/plugin-sdk/config-runtime")>(
...
vi.mock("synthios/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("synthios/plugin-sdk/runtime-env")>(
    "synthios/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("synthios/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("synthios/plugin-sdk/infra-runtime")>(
    "synthios/plugin-sdk/infra-runtime",
  );
  return {
    ...actual,
