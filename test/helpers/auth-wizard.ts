...
export function requireSynthiosAgentDir(): string {
  const agentDir = process.env.SYNTHIOS_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SYNTHIOS_AGENT_DIR not set");
  }
  return agentDir;
}
