  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.SYNTHIOS_HOME;
        delete env.SYNTHIOS_STATE_DIR;
        delete env.SYNTHIOS_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "synthios.mjs");
