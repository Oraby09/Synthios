...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/synthios/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/synthios");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/synthios/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SYNTHIOS_CONTROL_UI_BASE_PATH__ = "/synthios";
    const app = mountApp("/synthios/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/synthios");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/synthios/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("synthios.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "synthios.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/synthios" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("synthios.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/synthios",
    });
    expect(JSON.parse(localStorage.getItem("synthios.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/synthios";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/synthios");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/synthios#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/synthios");
    expect(app.settings.token).toBe("");
