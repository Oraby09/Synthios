...
import type { SynthiosApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as SynthiosApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as SynthiosApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as SynthiosApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as SynthiosApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as SynthiosApp);
    await loadConfig(host as unknown as SynthiosApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as SynthiosApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as SynthiosApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as SynthiosApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as SynthiosApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as SynthiosApp);
    await loadDevices(host as unknown as SynthiosApp);
    await loadConfig(host as unknown as SynthiosApp);
    await loadExecApprovals(host as unknown as SynthiosApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as SynthiosApp);
    await loadConfig(host as unknown as SynthiosApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as SynthiosApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as SynthiosApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__SYNTHIOS_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as SynthiosApp;
...
async function loadOverviewLogs(host: SynthiosApp) {
...
function buildAttentionItems(host: SynthiosApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.synthios.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as SynthiosApp, true),
    loadConfigSchema(host as unknown as SynthiosApp),
    loadConfig(host as unknown as SynthiosApp),
  ]);
}
