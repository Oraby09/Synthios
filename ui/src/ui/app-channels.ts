...
export async function handleWhatsAppStart(host: SynthiosApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: SynthiosApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: SynthiosApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: SynthiosApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: SynthiosApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: SynthiosApp): string {
...
function resolveGatewayHttpAuthHeader(host: SynthiosApp): string | null {
...
function buildGatewayHttpHeaders(host: SynthiosApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: SynthiosApp) {
...
export function handleNostrProfileToggleAdvanced(host: SynthiosApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: SynthiosApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: SynthiosApp) {
  const state = host.nostrProfileFormState;
